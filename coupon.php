...
$counts = $db -> getAll("SELECT cate_id,count(id) AS num FROM {$db_qq3479015851}coupon AS i WHERE status = '1' AND grade > '0' {$where} GROUP BY cate_id ");
$count = array();
$count['total'] = $db->getOne("SELECT count(id) FROM {$db_qq3479015851}coupon WHERE status = '1' AND grade > '0' {$where}");
...
if($id) {
	...
	$coupon  = $db -> getRow("SELECT * FROM `{$db_qq3479015851}coupon` WHERE id = '$id' AND status = '1' AND grade > '0'");
	if(!$coupon['id']) write_msg('该优惠券已失效或者尚未通过审核！','olmsg');
	
	if($action == 'print'){
		
		$db -> query("UPDATE `{$db_qq3479015851}coupon` SET prints = prints + 1 WHERE id = '$id'");
		
		globalassign();
		include qq3479015851_tpl('print');
	
	} else {

		$db -> query("UPDATE `{$db_qq3479015851}coupon` SET hit = hit + 1 WHERE id = '$id'");
		
		$coupon['content'] = replace_insidelink($coupon['content'],'coupon');
		
		$space = $db -> getRow("SELECT tname,address,busway,tel FROM `{$db_qq3479015851}member` WHERE userid = '$coupon[userid]'");
		$uid = $db -> getOne("SELECT id FROM `{$db_qq3479015851}member` WHERE userid = '$coupon[userid]'");
	...
	$coupon = page1("SELECT * FROM `{$db_qq3479015851}coupon` $where ORDER BY $orderby DESC");
	$list = array();
