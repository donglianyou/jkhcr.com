...
if($iflogin != 1){
    if($action == 'pay'){
        ...
        echo qq3479015851_goto('index.php?mod=login');
    }
}elseif($action == 'index'){
    $row = $db -> getRow("SELECT * FROM `{$db_qq3479015851}member` WHERE userid = '$s_uid'");
    $total['info'] = qq3479015851_count('information', "WHERE userid = '$s_uid'");
    $total['shoucang'] = qq3479015851_count('shoucang', "WHERE userid = '$s_uid'");
    if($row['if_corp'] == 1){
        $total['docu'] = qq3479015851_count('member_docu', "WHERE userid = '$s_uid'");
        $total['goods'] = qq3479015851_count('goods', "WHERE userid = '$s_uid'");
    }
    include qq3479015851_tpl('member');
    exit;
}
if($dopost == 1){
    switch($action){
    ...
    case 'editbase': if($if_corp == 1){
            ...
            $db -> query("UPDATE `{$db_qq3479015851}member` SET mobile='$mobile',email='$email',qq='$qq',tel='$tel',address='$address',catid='$catids',cityid='$cityid',areaid='$areaid',streetid='{$streetid}'{$tname}{$cname} WHERE userid = '$s_uid'");
            if($type == 'reg'){
            ...
            $db -> query("UPDATE `{$db_qq3479015851}member` SET mobile='$mobile',email='$email',qq='$qq'{$cname} WHERE userid = '$s_uid'");
        }
    ...
    case 'mypost': $row = $db -> getRow("SELECT money_own FROM `{$db_qq3479015851}member` WHERE userid = '$s_uid'");
        if($act == 'refreshid'){
            if(!$id){
                ...
                $activetime = $db -> getOne("SELECT activetime FROM `{$db_qq3479015851}information` WHERE id = '$id'");
                $endtime = $activetime == 0 ? 0 : $activetime * 3600 * 24 + $timestamp;
                $db -> query("UPDATE `{$db_qq3479015851}information` SET begintime = '$timestamp',endtime='$endtime' WHERE id ='$id'");
                msetcookie('refreshed' . $id, 1);
                if($delmoney) $db -> query("UPDATE `{$db_qq3479015851}member` SET money_own = money_own - '$delmoney' WHERE userid = '$s_uid'");
            ...
            if(!$id){
                ...
                $ifbold = $db -> getOne("SELECT ifbold FROM `{$db_qq3479015851}information` WHERE id = '$id'");
                if($ifbold == 1){
                    echo '操作失败，该信息标题已是加粗状态了';
                    exit;
                }
                $db -> query("UPDATE `{$db_qq3479015851}information` SET ifbold = '1' WHERE id ='$id'");
                if($delmoney) $db -> query("UPDATE `{$db_qq3479015851}member` SET money_own = money_own - '$delmoney' WHERE userid = '$s_uid'");
            ...
            if(!$id){
                ...
                $ifred = $db -> getOne("SELECT ifred FROM `{$db_qq3479015851}information` WHERE id = '$id'");
                if($ifred == 1){
                    echo '操作失败，该信息标题已是套红状态了';
                    exit;
                }
                $db -> query("UPDATE `{$db_qq3479015851}information` SET ifred = '1' WHERE id ='$id'");
                if($delmoney) $db -> query("UPDATE `{$db_qq3479015851}member` SET money_own = money_own - '$delmoney' WHERE userid = '$s_uid'");
            ...
            if(!$id){
                ...
                if($r[modid] > 1) qq3479015851_delete('information_' . $r[modid], "WHERE id = '$id'");
            ...
            if(!$info = $db -> getRow("SELECT  title,id,upgrade_type,upgrade_type_list,upgrade_type_index FROM `{$db_qq3479015851}information` WHERE id = '$id'")){
            ...
            if($row['money_own'] < $need){
                ...
                if($upgrade_type == 'cfg_member_upgrade_top'){
                    ...
                    $db -> query("UPDATE `{$db_qq3479015851}information` SET upgrade_type = '2' , upgrade_time = '$upgrade_time' WHERE id = '$id'");
                    ...
                    $db -> query("UPDATE `{$db_qq3479015851}information` SET upgrade_type_list = '2' , upgrade_time_list = '$upgrade_time' WHERE id = '$id'");
                    ...
                    $db -> query("UPDATE `{$db_qq3479015851}information` SET upgrade_type_index = '2' , upgrade_time_index = '$upgrade_time' WHERE id = '$id'");
                }
                $db -> query("UPDATE `{$db_qq3479015851}member` SET money_own = money_own - '$need' WHERE userid = '$s_uid'");
    ...
    switch($action){
    case 'upface': include qq3479015851_tpl('member_upface');
        break;
    case 'editbase': include qq3479015851_tpl('member_editbase');
        break;
    case 'editpwd': include qq3479015851_tpl('member_editpwd');
        break;
    case 'pay': $uid = $db -> getOne("SELECT id FROM `{$db_qq3479015851}member` WHERE userid = '$s_uid'");
    ...
    case 'mypost': require_once QQ3479015851_DATA . '/info.level.inc.php';
        ...
        if($act == 'upgrade'){
            ...
            $info = $db -> getRow("SELECT * FROM `{$db_qq3479015851}information` WHERE id = '$id'");
            $info[gname] = $db -> getOne("SELECT catname FROM `{$db_qq3479015851}category` WHERE catid = '$info[gid]'");
            include qq3479015851_tpl('member_mypost_upgrade');
        }else{
            ...
            $info_list = page1("SELECT * FROM `{$db_qq3479015851}information` $where ORDER BY begintime DESC", $perpage);
            $pageview = pager();
            include qq3479015851_tpl('member_mypost');
        }
        break;
    case 'shoucang': $sql = "SELECT a.*,b.id,b.catname FROM {$db_qq3479015851}shoucang AS a LEFT JOIN {$db_qq3479015851}information AS b ON a.infoid = b.id WHERE a.userid = '$s_uid' ORDER BY a.id DESC";
        $list = array();
        ...
        include qq3479015851_tpl('member_shoucang');
        break;
    case 'docu': $row = $db -> getRow("SELECT id FROM `{$db_qq3479015851}member` WHERE userid = '$s_uid'");
        ...
        $pageview = pager();
        include qq3479015851_tpl('member_docu');
        break;
    case 'goods': $where = " WHERE userid = '$s_uid'";
        ...
        $pageview = pager();
        include qq3479015851_tpl('member_goods');
        break;
    ...
    case 'upgradecorp': if($row['if_corp'] == 1){
            ...
            $db -> query("UPDATE `{$db_qq3479015851}member` SET if_corp = 1,status = 0 WHERE userid = '$s_uid'");
