...
$info = $db->getRow("SELECT a.*,b.areaname,c.streetname FROM `{$db_qq3479015851}information` AS a LEFT JOIN  `{$db_qq3479015851}area` AS b ON a.areaid = b.areaid LEFT JOIN `{$db_qq3479015851}street` AS c ON a.streetid = c.streetid WHERE a.id = '$id' AND a.info_level > 0");
if(!$info) write_msg('该信息不存在或已被删除！',$SystemGlobalcfm_global['SiteUrl']);
if(empty($info['info_level'])) write_msg('该信息尚未通过审核！',$SystemGlobalcfm_global['SiteUrl']);

$getrow = $db -> getRow("SELECT parentid,template_info,usecoin,catname,dir_typename,modid FROM `{$db_qq3479015851}category` WHERE catid = '$info[catid]'");
...
$info['contactview'] = ($info['endtime'] == '<font color=red>已过期</font>' && $SystemGlobalcfm_global['cfg_info_if_gq'] != 1) ? 0 : 1;
...
if($info['ismember']==1 && $info['userid']){
	...
	$member['if_corp'] = $SystemGlobalcfm_global['cfg_if_corp'] != 1 ? 0 : $member['if_corp'];
	$group  = get_member_group('',$info['userid']);
	if($member['userid'] && $group['member_contact'] == 0 && $info['ismember'] == '1'){
		$info['tel'] 		 = $SystemGlobalcfm_global['SiteTel'];
		$info['qq'] 		 = $SystemGlobalcfm_global['SiteQQ'];
		$info['email'] 		 = $SystemGlobalcfm_global['SiteEmail'];
	}
...
if(function_exists("gd_info") && $SystemGlobalcfm_global['cfg_info_if_img'] == 1){
	$info['qqnum'] 		= $info['qq']?	"<img src=\"".$SystemGlobalcfm_global['SiteUrl']."/".$SystemGlobalcfm_global['cfg_authcodefile']."?part=contact&wid=130&strings=".base64_encode($info['qq'])."\">":$info['qq'];
	$info['email'] 		= $info['email']?	"<img src=\"".$SystemGlobalcfm_global['SiteUrl']."/".$SystemGlobalcfm_global['cfg_authcodefile']."?part=contact&wid=230&strings=".base64_encode($info['email'])."\">":$info['email'];
	$info['telephone'] 	= $info['tel']	?	"<img src=\"".$SystemGlobalcfm_global['SiteUrl']."/".$SystemGlobalcfm_global['cfg_authcodefile']."?part=contact&wid=130&strings=".base64_encode($info['tel'])."\">":$info['tel'];
} else {
	$info['qqnum'] 		= $info['qq'];
...
if($info['modid'] > 1){
	$extr = $db ->getRow("SELECT * FROM `{$db_qq3479015851}information_{$info[modid]}` WHERE id ='$id'");
...
$info['image'] = $info['img_path'] != '' ? $db -> getAll("SELECT prepath,path FROM `{$db_qq3479015851}info_img` WHERE infoid = '$id' ORDER BY id ASC") : false;
...
$pdetail = ($info['img_path'] != '' ? '【图】':'').$info['title'].'-'.$city['cityname'].$info['areaname'].$info['streetname'].$info['catname'].'-'.$SystemGlobalcfm_global['SiteName'];
...
globalassign();
include qq3479015851_tpl($info['template_info']?$info['template_info']:'info');
