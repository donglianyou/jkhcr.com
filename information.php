...
if(!$info) write_msg('该信息不存在或已被删除！',$qq3479015851_global['SiteUrl']);
if(empty($info['info_level'])) write_msg('该信息尚未通过审核！',$qq3479015851_global['SiteUrl']);
...
$info['contactview'] = ($info['endtime'] == '<font color=red>已过期</font>' && $qq3479015851_global['cfg_info_if_gq'] != 1) ? 0 : 1;
...
if($info['ismember']==1 && $info['userid']){
	...
	$member['if_corp'] = $qq3479015851_global['cfg_if_corp'] != 1 ? 0 : $member['if_corp'];
	$group  = get_member_group('',$info['userid']);
	if($member['userid'] && $group['member_contact'] == 0 && $info['ismember'] == '1'){
		$info['tel'] 		 = $qq3479015851_global['SiteTel'];
		$info['qq'] 		 = $qq3479015851_global['SiteQQ'];
		$info['email'] 		 = $qq3479015851_global['SiteEmail'];
	}
...
if(function_exists("gd_info") && $qq3479015851_global['cfg_info_if_img'] == 1){
	$info['qqnum'] 		= $info['qq']?	"<img src=\"".$qq3479015851_global['SiteUrl']."/".$qq3479015851_global['cfg_authcodefile']."?part=contact&wid=130&strings=".base64_encode($info['qq'])."\">":$info['qq'];
	$info['email'] 		= $info['email']?	"<img src=\"".$qq3479015851_global['SiteUrl']."/".$qq3479015851_global['cfg_authcodefile']."?part=contact&wid=230&strings=".base64_encode($info['email'])."\">":$info['email'];
	$info['telephone'] 	= $info['tel']	?	"<img src=\"".$qq3479015851_global['SiteUrl']."/".$qq3479015851_global['cfg_authcodefile']."?part=contact&wid=130&strings=".base64_encode($info['tel'])."\">":$info['tel'];
} else {
	$info['qqnum'] 		= $info['qq'];
...
$pdetail = ($info['img_path'] != '' ? '【图】':'').$info['title'].'-'.$city['cityname'].$info['areaname'].$info['streetname'].$info['catname'].'-'.$qq3479015851_global['SiteName'];
