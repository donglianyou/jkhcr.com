...
function get_catparents()
{
	...
	if ($data === false) {
		$query = $db->query('SELECT * FROM `' . $db_qq3479015851 . 'category` WHERE if_view = \'2\' AND parentid = \'0\' ORDER BY catorder DESC');
...
{
	if ($row = $GLOBALS['db']->getAll('SELECT catid FROM `' . $GLOBALS['db_qq3479015851'] . $type . '` WHERE parentid = \'' . $catid . '\'')) {
		$cat = array();
...
{
	if ($row = $GLOBALS['db']->getAll('SELECT corpid FROM `' . $GLOBALS['db_qq3479015851'] . 'corp` WHERE parentid = \'' . $corpid . '\'')) {
		$cat = array();
...
function get_categories_foreach($catid = 0, $type = 'category', $ifview = '2')
{
	...
	$child = qq3479015851_count($type, 'WHERE parentid = \'' . $catid . '\'');
	if (!($child) || ($child == 0)) {
		$row = $GLOBALS['db']->getRow('SELECT parentid FROM `' . $GLOBALS['db_qq3479015851'] . $type . '` WHERE catid = \'' . $catid . '\'');
		$sql = 'SELECT catid, catname, catorder, if_view, dir_typename FROM `' . $GLOBALS['db_qq3479015851'] . $type . '` WHERE parentid = \'' . $row['parentid'] . '\' ORDER BY catorder ASC , catid ASC';
	}
	else {
		$sql = 'SELECT catid, catname, catorder, if_view, dir_typename FROM `' . $GLOBALS['db_qq3479015851'] . $type . '` WHERE parentid = \'' . $catid . '\' ORDER BY catorder ASC , catid ASC';
	}
...
function get_categories_tree($catid = 0, $type = 'category', $ifview = '2')
{
	...
	if ($catid == 0) {
		if ($data === false) {
			...
			$sql = 'SELECT a.*, b.catid AS childid, b.catname AS childname, b.color AS childcolor,b.dir_typename AS child_dir_typename,b.htmlpath AS child_htmlpath,b.dir_typename AS child_dir_typename FROM `' . $GLOBALS['db_qq3479015851'] . $type . '` AS a LEFT JOIN `' . $GLOBALS['db_qq3479015851'] . $type . '` AS b ON b.parentid = a.catid WHERE a.parentid = \'' . $parentid . '\' ' . $bif_view . ' ORDER BY a.catorder ASC , b.catorder ASC';
			$res = $GLOBALS['db']->getAll($sql);
			$cat_arr = array();
...
function get_corp_tree($catid = 0, $type = 'corp')
{
	...
	if ($catid == 0) {
		if ($data === false) {
			$parentid = 0;
			$sql = 'SELECT a.corpid, a.corpname, b.corpid AS childid, b.corpname AS childname FROM `' . $GLOBALS['db_qq3479015851'] . $type . '` AS a LEFT JOIN `' . $GLOBALS['db_qq3479015851'] . $type . '` AS b ON b.parentid = a.corpid WHERE a.parentid = \'' . $parentid . '\' ORDER BY a.corporder ASC , a.corpid ASC, b.corporder ASC';
			$res = $GLOBALS['db']->getAll($sql);
			$cat_arr = array();
		...
		if (($data === NULL) || empty($data[$catid])) {
			$parentid = $GLOBALS['db']->getOne('SELECT parentid FROM `' . $GLOBALS['db_qq3479015851'] . $type . '` WHERE corpid = \'' . $corpid . '\'');

			if ($parentid == 0) {
				$sql = 'SELECT a.corpid, a.corpname, a.corporder, b.corpid AS childid, b.corpname AS childname, b.corporder AS childorder FROM `' . $GLOBALS['db_qq3479015851'] . $type . '` AS a LEFT JOIN `' . $GLOBALS['db_qq3479015851'] . $type . '` AS b ON b.parentid = a.corpid WHERE a.corpid = \'' . $catid . '\' ORDER BY corporder ASC , corpid ASC, childorder ASC';
			}
			else {
				$sql = 'SELECT a.corpid, a.corpname, a.corporder, b.corpid AS childid, b.corpname AS childname, b.corporder AS childorder FROM `' . $GLOBALS['db_qq3479015851'] . $type . '` AS a LEFT JOIN `' . $GLOBALS['db_qq3479015851'] . $type . '` AS b ON b.parentid = a.catid WHERE b.parentid = \'' . $parentid . '\' ORDER BY corporder ASC , corpid ASC, childorder ASC';
			}
...
function get_parent_cats($type = 'category', $cat = '')
{
	...
	if ($data === false) {
		if ($type == 'category') {
			$arr = $GLOBALS['db']->getAll('SELECT catid, catname, parentid, dir_typename,dir_typename FROM `' . $GLOBALS['db_qq3479015851'] . 'category`');
		}
		else if ($type == 'channel') {
			$arr = $GLOBALS['db']->getAll('SELECT catid, catname, parentid, dir_typename FROM `' . $GLOBALS['db_qq3479015851'] . 'channel`');
		}
		else if ($type == 'corp') {
			$arr = $GLOBALS['db']->getAll('SELECT corpid,corpname,parentid FROM `' . $GLOBALS['db_qq3479015851'] . 'corp`');
		}
	}
	else {
		$arr = $data;
...
function cat_list($type = 'category', $catid = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
	...
	if ($data === false) {
		if (in_array($type, array('area', 'corp'))) {
			$sql = 'SELECT c.' . $type . 'id, c.' . $type . 'name, c.parentid, c.' . $type . 'order, COUNT(s.' . $type . 'id) AS has_children FROM `' . $GLOBALS['db_qq3479015851'] . $type . '` AS c LEFT JOIN `' . $GLOBALS['db_qq3479015851'] . $type . '` AS s ON s.parentid=c.' . $type . 'id GROUP BY c.' . $type . 'id ORDER BY c.parentid, c.' . $type . 'order ASC';
		}
		else if ($type == 'category') {
			$sql = 'SELECT c.catid, c.modid, c.dir_typename, c.dir_typename, c.catname,c.usecoin, c.parentid, c.if_view, c.catorder, c.template_info, COUNT(s.catid) AS has_children FROM `' . $GLOBALS['db_qq3479015851'] . 'category` AS c LEFT JOIN `' . $GLOBALS['db_qq3479015851'] . 'category` AS s ON s.parentid=c.catid GROUP BY c.catid ORDER BY c.parentid, c.catorder ASC';
		}
		else {
			$sql = 'SELECT c.catid, c.dir_typename, c.dir_typename, c.catname, c.parentid, c.if_view, c.catorder, COUNT(s.catid) AS has_children FROM `' . $GLOBALS['db_qq3479015851'] . $type . '` AS c LEFT JOIN `' . $GLOBALS['db_qq3479015851'] . $type . '` AS s ON s.parentid=c.catid GROUP BY c.catid ORDER BY c.parentid, c.catorder ASC';
		}
...
function get_cat_info($catid = 0, $type = 'category')
{
	global $db;
	global $db_qq3479015851;
	return $db->getRow('SELECT * FROM `' . $db_qq3479015851 . $type . '` WHERE catid = \'' . $catid . '\'');
}

function has_children($type = 'channel', $catid)
{
	return $GLOBALS['db']->getOne('SELECT catid FROM `' . $GLOBALS['db_qq3479015851'] . 'channel` WHERE parentid = \'' . $catid . '\'');
}
