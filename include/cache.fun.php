...
function get_changecity_cities(){
	...
	if($FinalArray === false){
		...
		$all = $db -> getAll("SELECT * FROM `{$db_qq3479015851}city` WHERE `status` = '1' ORDER BY displayorder ASC");
		if(is_array($all)){
			foreach($all as $key => $val){
...
function write_plugin_cache(){
	...
	$query = $db -> query("SELECT * FROM `{$db_qq3479015851}plugin`");
...
function get_mobile_settings(){
	...
	if($data === false){
		...
		$res = $db->getOne("SELECT value FROM `{$db_qq3479015851}config` WHERE type='mobile' AND description = 'mobile'");
...
function get_commentsettings(){
	...
	if($data === false){
		...
		$res = $db->getOne("SELECT value FROM `{$db_qq3479015851}config` WHERE type='comment' AND description = 'comment'");
		$res = $res ? ($charset == 'utf-8' ? utf8_unserialize($res) : unserialize($res)) : array();
...
function get_info_counts($cityid=''){
	...
	//今天发布信息数量
	$sql = "SELECT catid,count(*) AS num FROM {$db_qq3479015851}information $where group by catid";
...
function write_insidelink_cache(){
	...
	$query = $db -> query("SELECT * FROM `{$db_qq3479015851}insidelink`");
	while($row = $db -> fetchRow($query)){
...
function updateadvertisement(){
	global $timestamp;
	$query = $GLOBALS['db'] -> query("SELECT * FROM `{$GLOBALS['db_qq3479015851']}advertisement` WHERE available>'0' AND starttime<='$timestamp' AND type != 'normalad' ORDER BY displayorder ASC");
...
function write_cron_cache(){
	...
	$query = $db->query("SELECT * FROM `{$db_qq3479015851}crons` WHERE 1 OR nextrun = '0'");
	while($row = $db -> fetchRow($query)){
...
function write_qqlogin_cache(){
	global $db,$db_qq3479015851,$timestamp;
	$query = $db -> query("SELECT * FROM `{$db_qq3479015851}config` WHERE type = 'qqlogin'");
	while($row = $db -> fetchRow($query)){
...
function write_wxlogin_cache( )
{
	...
	global $db_qq3479015851;
	global $timestamp;
	$query = $db->query( "SELECT * FROM `".$db_qq3479015851."config` WHERE type = 'wxlogin'" );
	while ( $row = $db->fetchRow( $query ) )
	{
		$res[$row['description']] = $row['value'];
	}
...
function write_authcode_cache(){
	...
	$query = $db -> query("SELECT * FROM `{$db_qq3479015851}config` WHERE type = 'authcode'");
	while($row = $db -> fetchRow($query)){
		$res[$row['description']] = $row['value'];
	}
...
function write_checkanswer_cache(){
	$query = $GLOBALS['db'] -> query("SELECT * FROM `{$GLOBALS['db_qq3479015851']}checkanswer` ORDER BY id DESC");
...
function update_checkanswer_settings(){
	...
	$query = $db -> query("SELECT * FROM `{$db_qq3479015851}config` WHERE type = 'checkanswe'");
	while($row = $db -> fetchRow($query)){
		$res[$row['description']] = $row['value'];
	}
...
function update_config_cache(){
	$query = $GLOBALS['db'] -> query("SELECT description,value FROM `{$GLOBALS['db_qq3479015851']}config` WHERE type = 'config'");
	while($row = $GLOBALS['db']->fetchRow($query)){
		$res[$row['description']] = $row['value'];
	}
	$content = "<?php\r\n";
...
function get_cache_config(){
	...
	if($data === false){
		$query = $GLOBALS['db'] -> query("SELECT * FROM `{$GLOBALS['db_qq3479015851']}cache`");
		while($row = $GLOBALS['db']->fetchRow($query)){
...
function write_jswizard_cache(){
	...
	$query = $db -> query("SELECT * FROM `{$db_qq3479015851}jswizard`");
	while($row = $db -> fetchRow($query)){
		$res[$row['flag']]['id'] 			= $row['id'];
		$res[$row['flag']]['flag'] 			= $row['flag'];
...
function get_credit_score(){
	...
	if($data === false){
		...
		$query = $db -> query("SELECT * FROM `{$db_qq3479015851}config` WHERE type = 'credit_sco'");
		while($row = $db -> fetchRow($query)){
...
function get_tpl_index(){
	...
	if($data === false){
		...
		$query = $db -> query("SELECT * FROM `{$db_qq3479015851}config` WHERE type = 'tpl'");
		while($row = $db -> fetchRow($query)){
			$res[$row['description']] = $charset == 'utf-8' ? utf8_unserialize($row['value']) : unserialize($row['value']);
		}
...
function get_city_caches($cityid=0){
	...
	if($data === false){
		...
		if(!empty($cityid)){
			//分站下的地区
			$query = $db -> query("SELECT * FROM `{$db_qq3479015851}area` WHERE cityid = '$cityid' ORDER BY displayorder ASC");
			while($row = $db -> fetchRow($query)){
			...
			if(!empty($createin_areaid)){
				$query = $db -> query("SELECT * FROM `{$db_qq3479015851}street` WHERE areaid IN(".$createin_areaid.") ORDER BY displayorder ASC");
				while($row = $db -> fetchRow($query)){
		...
		//分站首页显示友情链接
		$query = "SELECT weblogo, webname, url, catid, cityid, ifindex FROM `{$db_qq3479015851}flink` WHERE 1 {$city_limit} AND ischeck = '2' ORDER BY ordernumber ASC";
		$res = $db->getAll($query);
		...
		//分站下的广告
		$query = $db -> query("SELECT advid,code FROM `{$db_qq3479015851}advertisement` WHERE 1 {$city_limit} AND available>'0' AND starttime<='$timestamp' AND type != 'normalad'");
		while($row = $db -> fetchRow($query)){
...
function get_category_dir(){
	...
	if($data === false){
		$query = $GLOBALS['db'] -> query("SELECT catid,dir_typename FROM `{$GLOBALS['db_qq3479015851']}category`");
		while($row = $GLOBALS['db']->fetchRow($query)){
