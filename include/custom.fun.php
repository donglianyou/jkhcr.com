...
function customtag($flag = '')
{
	...
	case 'info':
		...
		$sqlfrom = ',t.catname,t.dir_typename FROM `' . $db_qq3479015851 . 'information` AS t';
	...
	case 'news':
		...
		$datalist = qq3479015851_get_news($items, $catid, $if_img, strexists($jstemplate, '{catname}') ? 1 : 0, $if_commend, $orderby);
		$writedata = '';
	...
	case 'store':
		...
		$ifindex = $special[1];
		$iflist = $special[2];
		$datalist = qq3479015851_get_members($items, $levelid, $orderby, 1, $if_certify, $ifindex, $iflist, $areaid, $catid);
		$writedata = '';
	...
	case 'goods':
		...
		$datalist = qq3479015851_get_goods($items, 1, $shuxing, $catid, $areaid, $orderby);
		$writedata = '';
