...
if ($action == 'post') {
	...
	$coin = floor($score / $SystemGlobalcfm_global['cfg_score_fee']);

	if (empty($coin)) {
		write_msg('兑换失败，请重新设置有效的兑换积分');
	}

	$truescore = $coin * $SystemGlobalcfm_global['cfg_score_fee'];
	...
	include SysGlbCfm_ROOT . '/template/box/score_coin.html';
}
