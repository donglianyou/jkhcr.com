...
function get_member_docu($num=10,$userid='',$if_check=1,$typeid=0){
	...
	$num = $num ? "LIMIT 0,$num" : "";
	$query = $db -> query("SELECT a.*,b.id AS uid,b.tname FROM `{$db_qq3479015851}member_docu` AS a LEFT JOIN `{$db_qq3479015851}member` AS b ON a.userid = b.userid {$where} ORDER BY a.pubtime DESC {$num}");
...
function qq3479015851_chk_randcode($getcode='')
{
	global $timestamp;
	session_start();
...
{
	global $admin_id,$db_qq3479015851,$db,$timestamp;
	$db->query("INSERT INTO `{$db_qq3479015851}admin_record_action` (id,adminid,ip,pubdate,action) VALUES ('','$admin_id','".GetIP()."','$timestamp','$msg')");
}
...
{
	global $charset,$db,$db_qq3479015851;
...
function qq3479015851_goto($url='')
{
...
function is_member_info($id,$type='public')
{
	...
	$post = $db->getRow("SELECT a.*,b.modid FROM `{$db_qq3479015851}information` AS a LEFT JOIN `{$db_qq3479015851}category` AS b ON a.catid = b.catid WHERE a.id = '$id' $type");
	return $post;
}
...
{
	$msgs="未知错误，可能与你提交的参数有关<br /><br />获取更多帮助请前往<a href='http://blog.csdn.net/qq_35921430' target='_blank'>官方博客教程</a>";
...
//count total nums of the table of qq3479015851 sql
function qq3479015851_count($table,$where='')
{
	global $db,$db_qq3479015851;
	$count = $db->getOne("SELECT count(*) FROM {$db_qq3479015851}".$table." ".$where);
	if($count){
		return $count;
	}else{
		return 0;
...
function qq3479015851_delete($table,$where='')
{
	global $db,$db_qq3479015851;
	$delete = $db->query("DELETE FROM {$db_qq3479015851}".$table." ".$where);
	if($delete){return true;}else{return false;}
}

function qq3479015851_del_all($table,$id,$idor='id')
{
	global $db,$db_qq3479015851;
	$id = !empty($id) ? join(',', $id) : 0;
	$delete = $db->query("DELETE FROM {$db_qq3479015851}".$table." WHERE ".$idor." IN (".$id.")");
...
function get_member_docutype(){
	...
	if($data === false){
		$query = $GLOBALS['db'] -> query("SELECT * FROM `{$GLOBALS['db_qq3479015851']}member_docutype` WHERE ifview = 2 ORDER BY displayorder,typeid DESC");
...
/*百宝箱*/
function qq3479015851_get_lifebox($num='12',$cityid=NULL){
	global $db,$db_qq3479015851;
	$city_limit = $cityid ? " AND cityid = '$cityid'" : " AND cityid = '0'";
	$query = $db -> query("SELECT * FROM `{$db_qq3479015851}lifebox` WHERE if_view = 2 {$city_limit} ORDER BY displayorder DESC LIMIT 0,$num");
	while($row = $db -> fetchRow($query)){
...
/*网站公告*/
function qq3479015851_get_announce($num='12',$cityid=NULL){
	global $db,$db_qq3479015851,$timestamp;
	$city_limit = $cityid ? " AND cityid = '$cityid'" : " AND cityid = '0'";
	$query = $db -> query("SELECT * FROM `{$db_qq3479015851}announce` WHERE begintime<'$timestamp' AND (endtime='0' OR endtime>'$timestamp') {$city_limit} ORDER BY id DESC LIMIT 0,$num");
	while($row = $db -> fetchRow($query)){
...
/*便民电话*/
function qq3479015851_get_telephone($num='20',$cityid=NULL){
	global $db,$db_qq3479015851;
	
	$city_limit = $cityid ? " AND cityid = '$cityid'" : " AND cityid = '0'";
	
	$query = $db -> query("SELECT * FROM `{$db_qq3479015851}telephone` WHERE if_view = 2 {$city_limit} ORDER BY displayorder DESC LIMIT 0,$num");
	while($row = $db -> fetchRow($query)){
...
function get_seoset(){
	...
	if($data === false){
		$query = $GLOBALS['db']->query("SELECT description,value FROM `{$GLOBALS['db_qq3479015851']}config` WHERE type = 'seo'");
		while($row = $GLOBALS['db']->fetchRow($query)){
...
function get_areaname($areaid){
	...
	return $data[$areaid]['areaname'] ? $data[$areaid]['areaname'] : $db -> getOne("SELECT areaname FROM `{$db_qq3479015851}area` WHERE areaid = '$areaid'");
}
...
function verify_badwords_filter($ifopenname,$title='',$content=''){
	...
	if($res === NULL){
		...
		if($data === false){
			$query = $GLOBALS['db'] -> query("SELECT words,view,ifcheck FROM `{$GLOBALS['db_qq3479015851']}badwords`");
			while($row = $GLOBALS['db']->fetchRow($query)){
	...
	if($data === false){
		...
		while($row = $GLOBALS['db'] -> fetchRow($query)){
			...
			foreach($option as $w => $u){
				$newrow = $GLOBALS['db'] -> getRow("SELECT identifier,search FROM `{$GLOBALS['db_qq3479015851']}info_typeoptions` WHERE optionid='$u'");
				if($newrow['search']=='on'){
...
function mod_identifier(){
	...
	if($data === false){
		...
		while($row = $GLOBALS['db'] -> fetchRow($query)){
			$option = explode(',',$row[options]);
			foreach($option as $w => $u){
				$nrow = $GLOBALS['db'] -> getRow("SELECT optionid,title,identifier,type,rules,search FROM `{$GLOBALS['db_qq3479015851']}info_typeoptions` WHERE optionid='$u'");
				if($nrow['search'] == 'on'){
...
function qq3479015851_get_faq($num=5,$typeid=NULL){
	global $db,$db_qq3479015851,$seo;
	if(!$seo) $seo = get_seoset();
	$where  = $typeid ? "WHERE typeid = $typeid" : '';
	$sql 	= "SELECT id,typeid,title FROM {$db_qq3479015851}faq $where ORDER BY id DESC LIMIT 0,".$num;
	$do_qq3479015851 = $db -> query($sql);
	
	while($row = $db -> fetchRow($do_qq3479015851)){
		$arr['id']        	= $row['id'];
		$arr['title'] 		= $row['title'];
...
function qq3479015851_get_focus($type='index',$num=5,$cityid=NULL){
	global $db,$db_qq3479015851,$independency;
	...
	$query = $db -> query("SELECT image,pre_image,url,words,id FROM `{$db_qq3479015851}focus` WHERE 1 {$type_limit} {$city_limit} ORDER BY focusorder DESC LIMIT 0,$num");
	while($row = $db -> fetchRow($query)){
		$list[$row['id']]['id'] = $row['id'];
...
function qq3479015851_get_news($num=10,$catid=NULL,$ifimg=NULL,$leftjoin=NULL,$ifhot=NULL,$orderby=1){
	...
	$res = array();
	if($leftjoin){
		$query = $db -> query("SELECT a.*,b.catname FROM `{$GLOBALS['db_qq3479015851']}news` AS a LEFT JOIN `{$GLOBALS['db_qq3479015851']}channel` AS b ON a.catid = b.catid WHERE 1 {$cat_limit} {$img_limit}{$city_limit} {$commend_limit} {$orderby} LIMIT 0,$num");
		while($row = $db -> fetchRow($query)){
			$arr['id'] 			= $row['id'];
			$arr['title'] 		= $row['title'];
		...
		$query = $db -> query("SELECT a.* FROM `{$GLOBALS['db_qq3479015851']}news` AS a WHERE 1 {$cat_limit} {$img_limit}{$city_limit} {$orderby} LIMIT 0,$num");
		while($row = $db -> fetchRow($query)){
			$arr['id'] 			= $row['id'];
...
function qq3479015851_get_members($num=NULL,$level=NULL,$orderby=NULL,$if_certify=NULL,$ifindex=NULL,$iflist=NULL,$catid=NULL,$cityid=NULL){
	...
	if($qq3479015851_global['cfg_if_corp'] == 1){
		...
		$sql = "SELECT a.* FROM `{$db_qq3479015851}member` AS a  $where $orderby $limit";
...
/*获得机构文档/文章*/
function qq3479015851_get_member_docus($num=10,$userid=NULL,$typeid=NULL,$orderby=1){
	global $db,$db_qq3479015851;
	...
	$query = $db -> query("SELECT a.*,b.id AS uid,b.tname FROM `{$db_qq3479015851}member_docu` AS a LEFT JOIN `{$db_qq3479015851}member` AS b ON a.userid = b.userid {$where} {$orderby} {$limit}");
	while($row = $db -> fetchRow($query)){
		$arr['id'] = $row['id'];
		$arr['userid'] = $row['userid'];
		$arr['tname'] = $row['tname'];
...
function qq3479015851_get_infos($num=10,$info_level=NULL,$upgrade_type=NULL,$userid=NULL,$catid=NULL,$certify=NULL,$if_hot=NULL,$tel=NULL,$cityid=NULL){
	...
	$info_list = array();
	$idin = get_page_idin("id","SELECT a.id FROM `{$db_qq3479015851}information` AS a {$where} {$orderby}",$num);
	
	if($idin){
		$sql = "SELECT a.id,a.contact_who,a.title,a.content,a.begintime,a.catid,a.info_level,a.hit,a.dir_typename,a.ifred,a.ifbold,a.userid,a.catid,a.cityid,a.catname,a.img_path FROM `{$GLOBALS['db_qq3479015851']}information` AS a WHERE id IN ($idin) {$orderby}";
		$do_qq3479015851 = $db -> query($sql);
		while($row = $db -> fetchRow($do_qq3479015851)){
			$arr['id']        = $row['id'];
			$arr['catid']     = $row['catid'];
...
/*团购活动*/
function qq3479015851_get_groups($num=10,$glevel=NULL,$cityid=false)
{
	global $db,$db_qq3479015851;
	static $res = NULL;
	$limit = $num ? " LIMIT 0,$num" : "";
	...
	$query = $db -> query("SELECT * FROM `{$db_qq3479015851}group` $where ORDER BY dateline DESC $limit");
	while($row = $db -> fetchRow($query)){
...
/*商品*/
function qq3479015851_get_goods($num=10,$onsale=1,$shuxing=NULL,$catid=NULL,$userid=NULL,$orderby=1,$cityid=false)
{
	global $db,$db_qq3479015851;
	$limit = $num ? " LIMIT 0,$num" : "";
	...
	$where .= $catid ? " AND catid IN(".qq3479015851_get_goods_children($catid).")" : "";
	$where .= $cityid ? " AND cityid = '$cityid'" : "";
	
	$orderby = empty($orderby) ? " ORDER BY hit DESC " : " ORDER BY dateline DESC ";

	$query = $db -> query("SELECT * FROM `{$db_qq3479015851}goods` $where $orderby $limit");
	while($row = $db -> fetchRow($query)){
...
/*优惠券*/
function qq3479015851_get_coupons($num=10,$grade=0)
{
	global $db,$db_qq3479015851,$city;
	static $res = NULL;
	$limit = $num ? " LIMIT 0,$num" : "";
	$where = $grade ? " WHERE grade > '$grade'" : " WHERE grade > '0'";
	$where .= $cityid ? " AND cityid = '$cityid'" : "";
	$query = $db -> query("SELECT * FROM `{$db_qq3479015851}coupon` $where ORDER BY begindate DESC $limit");
	while($row = $db -> fetchRow($query)){
...
//获得导航  主导航 副导航 尾部导航
function qq3479015851_get_navurl($type,$num='30'){
	$data = read_static_cache('navurl_'.$type);
	if ($data === false){
		if($type == 'foot') {
			$query = "SELECT * FROM `{$GLOBALS['db_qq3479015851']}navurl` WHERE isview = '2' AND typeid = '2' ORDER BY displayorder ASC LIMIT 0,$num";
		} elseif($type == 'header') {
			$query = "SELECT * FROM `{$GLOBALS['db_qq3479015851']}navurl` WHERE isview = '2' AND typeid = '3' ORDER BY displayorder ASC LIMIT 0,$num";
		} elseif($type == 'head') {
			$query = "SELECT * FROM `{$GLOBALS['db_qq3479015851']}navurl` WHERE isview = '2' AND typeid = '1' ORDER BY displayorder ASC LIMIT 0,$num";
		}
...
function get_aboutus($id=NULL){
	...
	if(!$seo) $seo = get_seoset();
	if(!$id){
		$query = $GLOBALS['db']->query("SELECT id,typename,pubdate,dir_typename FROM `{$GLOBALS['db_qq3479015851']}about` ORDER BY displayorder,id ASC");
		while($row = $GLOBALS['db']-> fetchRow($query)){
			$arr['id'] 			= $row['id'];
		...
		$aboutus = $GLOBALS['db']->getRow("SELECT * FROM {$GLOBALS['db_qq3479015851']}about WHERE id =$id");
	}
	return $aboutus;
}
/*
...
function get_faq($id=NULL){
	global $seo,$db,$db_qq3479015851;
	if(!$seo) $seo = get_seoset();
	if($id){
		$faq = $db -> getRow("SELECT id,title,content FROM `{$db_qq3479015851}faq` WHERE id = '$id'");
	}else{
		$query = $db -> query("SELECT id,typename FROM `{$db_qq3479015851}faq_type` ORDER BY id ASC");
		while($row = $db->fetchRow($query)){
			$faq[$row['id']]['typename'] = $row['typename'];
		}
		$query = $row = NULL;
		
		$query = $db ->query("SELECT * FROM `{$db_qq3479015851}faq` ORDER BY id DESC");
		while($row = $db -> fetchRow($query)){
			$arr['id'] 		= $row['id'];
			$arr['title'] 	= $row['title'];
...
function get_flink(){
	...
	$query = $db -> query("SELECT * FROM `{$db_qq3479015851}flink_type` ORDER BY id ASC");
	while($row = $db -> fetchRow($query)){
		$flink[$row['id']]['typename'] = $row['typename'];
	}
	$query = $row = NULL;
	
	$query = $db -> query("SELECT * FROM `{$db_qq3479015851}flink` WHERE ischeck = '2' AND ifindex = '1' {$city_limit} ORDER BY weblogo DESC,ordernumber DESC,id DESC");
	while($row = $db -> fetchRow($query)){
		$arr['id'] = $row['id'];
...
function get_member_info($userid=''){
	$row = $GLOBALS['db'] -> getRow("SELECT * FROM `{$GLOBALS['db_qq3479015851']}member` WHERE userid = '$userid'");
...
{
	global $db,$db_qq3479015851,$s_uid;
	$member  = get_member_group('',$s_uid);
...
function get_member_group($groupid='',$userid=''){
	if(empty($groupid)){
		$groupid = $GLOBALS['db'] -> getOne("SELECT levelid FROM {$GLOBALS['db_qq3479015851']}member WHERE userid = '$userid'");
	}
	$data = read_static_cache('member_'.$groupid);
	if($data === false){
		$res = 	$GLOBALS['db'] -> getRow("SELECT * FROM `{$GLOBALS['db_qq3479015851']}member_level` WHERE id = '$groupid'");
		$res['allow_tpl'] = empty($res['allow_tpl']) ? '/template/space/theme1' : $res['allow_tpl'];
		
		$in = create_in(explode(',',$res['allow_tpl']),'tpl_path');
		
		$query = $GLOBALS['db'] -> query("SELECT * FROM `{$GLOBALS['db_qq3479015851']}member_tpl` WHERE $in AND if_view = '2'");
		$res['allow_tpl'] = array();
		while($row = $GLOBALS['db']->fetchRow($query)){
...
function qq3479015851_get_goods_children($catid)
{
	if($row = $GLOBALS['db']->getAll("SELECT catid FROM `{$GLOBALS['db_qq3479015851']}goods_category` WHERE parentid = '$catid'")){
		$cat = array();
		foreach ($row as $k => $v){
			$cat[$v['catid']] = $v['catid'];
		}
...
function get_smplist_cats($cats,$showstyle){
	...
	$query = $db -> query("SELECT * FROM `{$db_qq3479015851}category` WHERE parentid ".create_in($cats)." AND if_view = '2' ORDER BY catorder ASC");
...
function select_where($table,$name='parentid',$currentid='',$parentid=''){
	...
	if($parentid){
		switch($table){
			...
			break;
			case 'street':
				$cityid = $db -> getOne("SELECT cityid FROM `{$db_qq3479015851}area` WHERE areaid = '$parentid'");
...
function get_latlng2cityid($lat='',$lng=''){
	global $db,$db_qq3479015851,$charset;
	if($lat && $lng){
...
function get_gid($catid){
	...
	if(empty($sid)){
		...
		$fid = $db->getOne("SELECT parentid FROM `{$db_qq3479015851}category` WHERE catid = '$sid'");
		if(empty($fid)){
			return $sid;
		}else{
			$did = $db->getOne("SELECT parentid FROM `{$db_qq3479015851}category` WHERE catid = '$fid'");
			if(empty($did)){
				return $fid;
			}
		}
...
function qq3479015851_chk_smsrandcode($getcode='',$phonenum='')
{
	global $timestamp;
	session_start();
	$getcode = trim(strtoupper($getcode));
