...
<dt>区域查找：</dt>
<dd>
    <?php if(is_array($area_list)){foreach($area_list as $SystemGlobalcfm) { ?><div><a href="<?=$city['domain']?><?=$SystemGlobalcfm['uri']?>" <? if($SystemGlobalcfm['select'] == 1) { ?>class="curr"<?php } ?>><?=$SystemGlobalcfm['areaname']?></a></div>
<?php }} ?>
</dd>
</dl>
<?php } ?>
...
<div><a href="#" class="curr">不限</a></div>
        <?php if(is_array($hotcities)){foreach($hotcities as $SystemGlobalcfm) { ?><div><a href="<?=$SystemGlobalcfm['domain']?><?=$cat['caturi']?>"><?=$SystemGlobalcfm['cityname']?></a></div>
<?php }} ?>
<div><a href="<?=$SystemGlobalcfm_global['SiteUrl']?>/changecity.php">更多 &raquo;</a></div>
</dd>
</dl>
<?php } ?>
<dl class="lastdl">
...
<div class="list_zpqz">
<ul>
        <?php if(is_array($info_list)){foreach($info_list as $SystemGlobalcfm) { ?><div class="hover <? if($SystemGlobalcfm['upgrade_type']>1) { ?>ding<?php } ?>">
<span class="ltitle">
<a href="<?=$SystemGlobalcfm['uri']?>" target="_blank" style="<? if($SystemGlobalcfm['ifred'] == 1) { ?>color:red;<?php } ?> <? if($SystemGlobalcfm['ifbold'] == 1) { ?>font-weight:bold;<?php } ?>" title="<?=$SystemGlobalcfm['title']?>"><? echo cutstr($SystemGlobalcfm['title'],18); ?></a>
        <? if($SystemGlobalcfm['img_count']) { ?><span class="img_count"><?=$SystemGlobalcfm['img_count']?>图</span><?php } if($SystemGlobalcfm['info_level'] == 2) { ?><span class="tuijian">荐</span><?php } if($SystemGlobalcfm['certify'] == 1) { ?><span class="certify">认证</span><?php } ?>
        <div class="inforbox">
          <div class="arrow"> <em></em> <span></span> </div>
          <div class="tipsbox">
            <h3><? echo cutstr($SystemGlobalcfm['company'],28); ?></h3>
            <ul class="lists">
              <li> <span>薪资待遇：</span> <font color="#F70"><? if($SystemGlobalcfm['day_salary']) { echo str_replace('.00','',$SystemGlobalcfm['day_salary']); ?><?php } else { echo str_replace('.00','',$SystemGlobalcfm['salary']); ?>元<?php } ?></font> </li>
              <li> <span>工作地点：</span> <?=$SystemGlobalcfm['areaname']?> </li>
              <? if($SystemGlobalcfm['work_life']) { ?><li> <span>工作经验：</span> <? echo str_replace('.00','',$SystemGlobalcfm['work_life']); ?> </li><?php } ?>
              <? if($SystemGlobalcfm['education']) { ?><li> <span>学历要求：</span> <?=$SystemGlobalcfm['education']?> </li><?php } ?>
              <? if($SystemGlobalcfm['job']) { ?><li> <span>招聘职位：</span> <?=$SystemGlobalcfm['job']?> </li><?php } ?>
              <? if($SystemGlobalcfm['property']) { ?><li> <span>公司性质：</span> <?=$SystemGlobalcfm['property']?></li><?php } ?>
              <? if($SystemGlobalcfm['fuli']) { ?><li style="width:400px;"><?=$SystemGlobalcfm['fuli']?></li><?php } ?>
            </ul>
            <dl class="content">
              <? echo cutstr($SystemGlobalcfm['content'],200,'...'); ?>            </dl>
            <dl class="warning">
              <strong><?=$SystemGlobalcfm_global['SiteName']?>提醒：</strong> 在签订合同之前，以各种理由收取非正规费用（押金、服装费、报名费、体检费、要求购买充值卡等）均有骗子嫌疑，请提高警惕。
            </dl>
            </div>
          </div>
        </span>
        <span class="ltime"><? echo get_format_time($SystemGlobalcfm['begintime']); ?></span>
        <span class="larea"><?=$SystemGlobalcfm['areaname']?></span>
<span class="lsalary"><? if($SystemGlobalcfm['day_salary']) { echo str_replace('.00','',$SystemGlobalcfm['day_salary']); ?><?php } else { echo str_replace('.00','',$SystemGlobalcfm['salary']); ?>元<?php } ?></span>
        <span class="lcompany"><? echo cutstr($SystemGlobalcfm['company'],28); ?>&nbsp;</span>
</div>
