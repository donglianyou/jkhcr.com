...
/*分类信息系统
联系QQ：3479015851*/?>
<? if($action == 'index' && $store['banner']) { ?><div class="gbanner" style='background-image:url("<?=$SystemGlobalcfm_global['SiteUrl']?><?=$store['banner']?>")'></div><?php } ?>
<div class="infobox">
    <div class="infoboximg"   pageTitle="商家相册">
        <a href="javascript:void(0)"><img src='<?=$store['prelogo']?>' onerror="this.src='<?=$SystemGlobalcfm_global['SiteUrl']?>/images/nophoto.jpg'"/></a>
    </div>
       
    <ul class="gsmc">
        <li><b><? echo $store['tname'] ? $store['tname'] : $store['userid'];; ?></b></li> 
        <li>
           <span class="dp_xx"><img src="<?=$SystemGlobalcfm_global['SiteUrl']?>/images/credit/<?=$store['credits']?>.gif"></span>
        </li>
        <li>
...
<div class="select_01" id="wrapper2">
    <ul class="tab-hd" id="scroller2">
        <?php if(is_array($navi)){foreach($navi as $k => $SystemGlobalcfm) { ?>        <li id="ele_<?=$k?>" class="item <? if($action == $k) { ?>current<?php } ?>"><a href="index.php?mod=store&id=<?=$store['id']?>&action=<?=$k?>"><?=$SystemGlobalcfm?></a></li>
        <?php }} ?>
    </ul>
</div>
<div class="clear"></div>
