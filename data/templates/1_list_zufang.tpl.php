...
<ul><?php if(is_array($info_list)){foreach($info_list as $SystemGlobalcfm) { ?><div class="hover <? if($SystemGlobalcfm['upgrade_type']>1) { ?>ding<?php } ?>">
<span class="lfaceimg"><a href='<?=$SystemGlobalcfm['uri']?>' target='_blank'><img src='<? if(!$SystemGlobalcfm['img_path']) { ?><?=$SystemGlobalcfm_global['SiteUrl']?>/images/nophoto.gif<?php } else { ?><?=$SystemGlobalcfm_global['SiteUrl']?><?=$SystemGlobalcfm['img_path']?><?php } ?>' title='<?=$SystemGlobalcfm['title']?>'></a></span>
<span class="ltitlevalue">
<a href="<?=$SystemGlobalcfm['uri']?>" target="_blank" style="<? if($SystemGlobalcfm['ifred'] == 1) { ?>color:red;<?php } ?> <? if($SystemGlobalcfm['ifbold'] == 1) { ?>font-weight:bold;<?php } ?>" title="<?=$SystemGlobalcfm['title']?>"><? echo cutstr($SystemGlobalcfm['title'],42); ?></a><span class='highlight'>[<?=$SystemGlobalcfm['position']?>]</span><? if($SystemGlobalcfm['img_count']) { ?><span class="img_count"><?=$SystemGlobalcfm['img_count']?>图</span><?php } if($SystemGlobalcfm['info_level'] == 2) { ?><span class="tuijian">推荐</span><?php } if($SystemGlobalcfm['certify'] == 1) { ?><span class="certify">认证</span><?php } ?>&nbsp;<br /><font style="font-size:12px; color:#808080"><? echo cutstr($SystemGlobalcfm['content'],70); ?></font><br />
<font class="xx1"><?=$SystemGlobalcfm['areaname']?></font><font class="xx2"><?=$SystemGlobalcfm['rent_type']?></font>
</span>
<span class="ltime"><? echo get_format_time($SystemGlobalcfm['begintime']); ?></span>
<span class="lprice"><span class="rprice"><? if(in_array($SystemGlobalcfm['mini_rent'],array('0元','0.00元'))) { ?>面议<?php } else { echo (float)$SystemGlobalcfm['mini_rent']; ?>元<?php } ?></span><br /><?=$SystemGlobalcfm['room_type']?></span>
</div>
<?php }} ?>
</ul>
