...
<title><?=$typename?>-<?=$store['tname']?>-<?=$SystemGlobalcfm_global['SiteName']?></title> 
...
<table class="mrw_list"> 
   ...
   <th align="left">发布时间</th>
                       ...
                       <?php if(is_array($docu)){foreach($docu as $SystemGlobalcfm) { ?>   <tr> 
   <td><a href="<?=$SystemGlobalcfm['uri']?>"><?=$SystemGlobalcfm['title']?></a></td> 
   <td align="left"><? echo GetTime($SystemGlobalcfm['pubtime'],'Y-m-d'); ?></td> 
                       <td align="left"><?=$SystemGlobalcfm['hit']?>次</td> 
   </tr>
                       <?php }} else {{ ?>
                       <tr> 
...
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title><?=$docu['title']?>-<?=$store['tname']?>-<?=$SystemGlobalcfm_global['SiteName']?></title> 
<link href="<?=$SystemGlobalcfm_global['SiteUrl']?>/template/spaces/store/css/<?=$store['template']?>.css" type="text/css" rel="stylesheet" />
</head>
<body>
<?php include qq3479015851_tpl('header'); ?>
