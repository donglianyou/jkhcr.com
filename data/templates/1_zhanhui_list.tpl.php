...
<script type="text/javascript">uaredirect("<?=$SystemGlobalcfm_global['SiteUrl']?>/m/index.php?mod=news&catid=<?=$catid?>&cityid=<?=$city['cityid']?>");</script>
...
<ul class="subcls schitem"> 
                        <?php if(is_array($cat_list)){foreach($cat_list as $SystemGlobalcfm) { ?><li <? if($SystemGlobalcfm['catid'] == $channel['catid']) { ?>class='thiscls'<?php } ?>><a href="<?=$SystemGlobalcfm['uri']?>">全部</a></li>
                        <?php if(is_array($SystemGlobalcfm['children'])){foreach($SystemGlobalcfm['children'] as $w) { ?>                        <li <? if($w['catid'] == $channel['catid']) { ?>class='thiscls'<?php } ?>><a href="<?=$w['uri']?>"><?=$w['catname']?></a></li> 
                        <?php }} ?>
<?php }} ?>
</ul> 
</div>
...
<div class="nowlist">
<ul>
                        <?php $i=1; ?>                            <?php if(is_array($news)){foreach($news as $SystemGlobalcfm) { ?><li><span class="title"><a href="<?=$SystemGlobalcfm['uri']?>" class="title" target="_blank" <? if($SystemGlobalcfm['iscommend'] == 1) { ?>style="color:red"<?php } ?>><?=$SystemGlobalcfm['title']?></a></span><span class="time"><? echo GetTime($SystemGlobalcfm['begintime'],'y-m-d'); ?></span></li>
