...
<input autocomplete="off" type="text" onBlur="chk_authcode()" id="checkcode" name="checkcode" class="passport-login-input passport-login-input-username"  style="width:130px;" placeholder="图中验证码" size="20" maxlength="75">
                    ...
                    <div style="margin-top:10px; text-align:center">
                    	<img src="<?=$SystemGlobalcfm_global['SiteUrl']?>/<?=$SystemGlobalcfm_global['cfg_authcodefile']?>?mod=m" alt="看不清，请点击刷新" align="absmiddle" style="cursor:pointer; border:1px #ddd solid;" onClick="this.src=this.src+'?'"/>
                    </div>
<?php } ?>
...
<li class="bor" style=" margin:0 auto; float:none; border:none"><a href="javascript:void(0);" onClick="usewxlogin();"><i class="hzico-wx"></i>使用微信账号登录</a></li>
                ...
                <?php } ?>
              	<? if($qqlogin['open'] == 1) { ?>
<ul>
<li class="bor" style=" margin:0 auto; float:none; border:none"><a href="<?=$SystemGlobalcfm_global['SiteUrl']?>/include/qqlogin/qq_login.php?mod=m"><i class="hzico-qq"></i>使用QQ账号登录</a></li>
</ul>
                <?php } ?>
  </div>
  </div>
