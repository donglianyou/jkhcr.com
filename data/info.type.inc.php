...
function get_info_var_type($type, $name = '', $rules = '', $value = '', $forward = 'back', $title = '', $require = '')
{
	...
	if ($forward == 'back') {
		...
		case 'text':
			...
			$qq3479015851 .= '<input maxlength="' . $rules[maxlength] . '" name="extra[' . $name . ']" value="' . $value . '"  class="input input-large" type="text" size="40" ' . $required . '> ' . $qq3479015851_rule_str;
			break;

		case 'textarea':
			$qq3479015851_rule_str = ($rules[maxlength] ? '<br />不得超过' . $rules[maxlength] . '个字符' : '');
			$qq3479015851 = '<textarea  maxlength="' . $rules[maxlength] . '" name="extra[' . $name . ']"  cols="61" rows="10" class="input" ' . $required . '>' . $value . '</textarea> ' . $qq3479015851_rule_str;
			break;
		...
		case 'select':
			$qq3479015851 .= '<select name="extra[' . $name . ']" class="input" ' . $required . '>';
		...
		case 'number':
			...
			$qq3479015851 .= '<input ' . $required . ' msgid="' . $title . '" name="extra[' . $name . ']" value="' . $value . '" type="text" style="width:50px"  class="input"> <span class=units>' . $rules[units] . '</span>';
			break;
		}
	}
	else {
		...
		case 'text':
			$qq3479015851_rule_str = ($rules[maxlength] ? $rules[maxlength] . '字符以内' : '');
			$qq3479015851 .= '<input maxlength="' . $rules[maxlength] . '" name="' . $name . '" value="' . $value . '"  class="input input-large" type="text" size="40"> ' . $qq3479015851_rule_str;
			break;

		case 'textarea':
			$qq3479015851_rule_str = ($rules[maxlength] ? '<br />不得超过' . $rules[maxlength] . '个字符' : '');
			$qq3479015851 = '<textarea maxlength="' . $rules[maxlength] . '" name="' . $name . '"  cols="100" rows="10" class="input">' . $value . '</textarea> ' . $qq3479015851_rule_str;
			break;
		...
		case 'select':
			$qq3479015851 .= '<select name="' . $name . '" class="input" >';
			$qq3479015851 .= '<option value="">请选择' . $title . '</option>';
		...
		case 'number':
			$qq3479015851 .= '<input name="' . $name . '[min]" class="input" type="text" style="width:50px"> 至 <input  name="' . $name . '[max]" class="input" type="text" style="width:50px; float:none;"> ';
			break;
		}
	}
...
function return_category_info_options($modid = '', $edit_id = '')
{
	...
	if (1 < $modid) {
		...
		if ($row[options]) {
			$resquery = $db->query('SELECT optionid,title,identifier,type,rules,required FROM `' . $db_qq3479015851 . 'info_typeoptions` WHERE optionid IN (' . $row['options'] . ')');
