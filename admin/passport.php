...
if (!submit_check(CURSCRIPT . '_submit')) {
	...
	if ($part == 'bbs') {
		$query = $db->query('SELECT * FROM `' . $db_qq3479015851 . 'config` WHERE type = \'passport\'');
	...
	$db->query('DELETE FROM `' . $db_qq3479015851 . 'config` WHERE type = \'passport\'');
	...
	if ($passport_type == 'none') {
		...
		if (is_array($ucsettings)) {
			...
			foreach ($ucsettings as $key => $val ) {
				$db->query('INSERT INTO `' . $db_qq3479015851 . 'config` (`description`,`value`,`type`)VALUES(\'' . $key . '\',\'' . $val . '\',\'passport\');');
			}
		}
	}
	...
	if (is_array($wxsettings)) {
		$db->query('DELETE FROM `' . $db_qq3479015851 . 'config` WHERE type = \'wxlogin\'');

		foreach ($wxsettings as $key => $val ) {
			$db->query('INSERT INTO `' . $db_qq3479015851 . 'config` (`description`,`value`,`type`)VALUES(\'' . $key . '\',\'' . $val . '\',\'wxlogin\');');
		}
	}
	...
	if (is_array($qqsettings)) {
		$db->query('DELETE FROM `' . $db_qq3479015851 . 'config` WHERE type = \'qqlogin\'');

		foreach ($qqsettings as $key => $val ) {
			$db->query('INSERT INTO `' . $db_qq3479015851 . 'config` (`description`,`value`,`type`)VALUES(\'' . $key . '\',\'' . $val . '\',\'qqlogin\');');
		}
	}
