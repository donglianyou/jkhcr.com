    ...
    <div class="mpstopic-category">
        <div class="panel-tab">
            <ul class="clearfix tab-list">
                <li><a title="已添加的展会类别" href="channel.php">已添加的展会类别</a></li>
                <li><a title="新增展会类别" href="channel.php?part=add">新增展会类别</a></li>
				<li><a title="编辑展会类别" href="channel.php?part=edit&catid=<?=$catid?>" class="current">编辑展会类别</a></li>
            </ul>
        </div>
    </div>
</div>
...
<tr bgcolor="#f5fbff">
  ...
  <td><input name="catname" type="text" class="text" id="catname" value="<?=$cat[catname]?>" size="30"> 
          ...
          <?foreach ($cat_color as $k=>$v){?>
          <option value="<?=$k?>" style="background-color:<?=$k?>;" <?if($cat[color] == $k) echo 'selected';?>><?=$v?></option>
          <?}?>
        </select>
...
<tr bgcolor="#f5fbff">
  ...
  <td> <input name="title" type="text" class="text" id="title" value="<?=$cat[title]?>" size="50"> <font color="red">*</font>(<font style="color:#FF6600">范例：郑州本地展会_郑州电脑展会</font>,不超过15个字符)
  </td>
</tr>
<tr bgcolor="#f5fbff">
  <td>关键词(keyword)： </td>
...
<center>
<input type="submit" name="<?=CURSCRIPT?>_submit" value="保存修改" class="qq3479015851 mini" />　
<input type="button" onClick="location.href='?part=list'" value="返 回" class="qq3479015851 mini">
</center>
</form>
