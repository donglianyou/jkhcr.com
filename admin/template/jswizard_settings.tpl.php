...
<table border="0" cellspacing="0" cellpadding="0" class="vbm">
    ...
    <tbody style="display: yes; background-color:white">
        <tr>
            <td width="45%" class="altbg1" ><b>启用外部调用:</b><br /><span class="smalltxt">外部调用将使得您可以将<?=$qq3479015851_global['SiteName']?>最新分类信息主题、排行等资料嵌入到您的普通网页中，访问者无需访问<?=$qq3479015851_global['SiteName']?>即可获知<?=$qq3479015851_global['SiteName']?>最近更新的情况</span></td><td class="altbg2"><label for="1"><input class="radio" type="radio" name="settingsnew[jsstatus]" value="1" id="1" <?php if($settings[jsstatus] == 1){echo 'checked';} ?> 
    ...
    <tr>
        <td width="45%" class="altbg1" valign="top"><b>外部调用数据来路限制:</b><br /><span class="smalltxt">为了避免其它网站非法调用<?=$qq3479015851_global['SiteName']?>数据，加重您的服务器负担，您可以设置允许调用<?=$qq3479015851_global['SiteName']?>数据的来路域名列表，只有在列表中的域名和网站，才能调用您<?=$qq3479015851_global['SiteName']?>的信息。每个域名一行，不支持通配符，请勿包含 http:// 或其它非域名内容，留空为不限制来路，即任何网站均可调用</span></td>
