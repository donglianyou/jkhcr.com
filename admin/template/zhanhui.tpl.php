	...
	<div class="mpstopic-category">
		<div class="panel-tab">
			<ul class="clearfix tab-list">
				<li><a href="zhanhui.php" <?php if($part == 'list'){?>class="current"<?php }?>>展会列表</a></li>
				<li><a href="zhanhui.php?part=add" <?php if($part == 'add'){?>class="current"<?php }?>>添加展会</a></li>
				
				<?php if($part == 'edit' && $row[id]){?><li><a href="zhanhui.php?part=edit&id=<?=$row[id]?>" class="current">编辑展会</a></li><?php }?>
			</ul>
		</div>
	</div>
</div>
...
<table border="0" cellspacing="0" cellpadding="0" class="vbm">
  ...
  <tr bgcolor="#ffffff">
    <td id="menu_tip">
  <li>若选择了焦点图轮显，请务必上传或指定缩略图路径，否则将无法成功发布展会</li>
...
<input type="hidden" name="html_path" value="<?php echo $row[html_path] == '/html/zhanhui'? '' : $row['html_path']?>">
<div id="<?=SysGlbCfm_SOFTNAME?>">
<table border="0" cellspacing="0" cellpadding="0" class="vbm">
<tr class="firstr">
...
<tr bgcolor="#f5fbff">
  ...
  <?php echo cat_list('zhchannel',0,$row[catid]);?>
</select></td>
  </tr>
...
<tr style="background-color:#FFF;">
  ...
  <td colspan="2">
    ...
    <label for="zhanhuifocus"><input name="isfocus[]" type="checkbox" class="checkbox" id="zhanhuifocus" value="zhanhui"  <?php if($row[isfocus] == 'zhanhui') echo 'checked';?>/>展会页</label>
  </td>
</tr>
<?php if($part == 'add'){?>
<tr id="pictable" style=" background-color:#FFF">
    ...
    <td>
        ...
        <iframe src="include/upfile.php?destination=zhanhui&watermark=0" width="450" frameborder="0" scrolling="no" onload="this.height=iFrame1.document.body.scrollHeight" id="iframe" style="display:none; margin-top:10px"></iframe>
    </td>
</tr>
<?php } else {?>
<tr id="pictable" style=" background-color:#FFF">
...
<center style='margin-bottom:10px'><input name="zhanhui_submit" value="<?php echo $part == 'edit'? '保存修改' : '提交发布'?>" type="submit" class="qq3479015851"/>&nbsp;&nbsp;<input name="zhanhui_submit" value="返 回" type="button" onClick="location.href='?part=list'" class="qq3479015851"/></center>
</form>
