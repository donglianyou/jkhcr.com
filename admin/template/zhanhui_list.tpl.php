...
<? }?>
<input type="submit" class="gray mini" value="检索展会"> 
</form>
			</ul>
		</div>
	</div>
...
<?php foreach($zhanhui AS $row){?>
    <tr bgcolor="white" >
    <td width="40"><input type='checkbox' name='delids[]' value="<?=$row['id']?>" class='checkbox' id="<?=$row['id']?>"></td>
    <td width="40"><?=$row['cityname']?></td>
    <td width="70"><a href="../zhanhui.php?catid=<?=$row['catid']?>"><?=$row['catname']?></a></td> 
    <td align="left"><a href="../zhanhui.php?id=<?=$row[id]?>" target="_blank" title="<?=$row['title']?>"><?=cutstr($row['title'],41)?></a></td>
...
<center><input type="submit" value="提 交" class="qq3479015851 large" name="zhanhui_submit"/></center>
</form>
<div class="clear"></div>
