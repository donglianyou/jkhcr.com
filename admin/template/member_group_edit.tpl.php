...
<input type="hidden" name="id" value="<?=$group[id]?>">
  ...
  <?=qq3479015851_member_purview($purviews)?>
  <tr class="firstr">
  ...
  <tr bgcolor="#f5fbff">
    ...
    <td ><input name="money_own" type="text" class="txt" value="<?=$group['money_own']?>"/> <img src="../member/images/qq3479015851_icon_incomes.gif" align="absmiddle"> </td>
  </tr>
  <?php }?>
  <tr bgcolor="#f5fbff">
...
<input name="settings[ifopen][<?php echo $k; ?>]" type="checkbox" class="checkbox" value="1" <?php if($settings['ifopen'][$k] == 1){?>checked="checked"<?php }?>>启用 <img src="../member/images/qq3479015851_icon_incomes.gif" align="absmiddle"> <input name="settings[money][<?php echo $k; ?>]" class="txt" value="<?php echo $settings['money'][$k]?>"> <?php echo $v; ?>
    </div>
    <?php }?>
   </td>
  </tr>
...
<font color="#006acd">（只对商家会员有效）</font></td>
  ...
  <tr bgcolor="#f5fbff">
    <td height="60">&nbsp;</td>
    <td>
    <input type="submit" name="Submit" class="qq3479015851 mini" value="确认提交"/>
    　<input type="button" onClick=history.back() class="qq3479015851 mini" value="返 回">           </td>
  </tr>  
    </form>
    </table>
</div>
