    ...
    <div class="mpstopic-category">
        <div class="panel-tab">
            <ul class="clearfix tab-list">
                <li><a title="已添加的展会类别" href="zhchannel.php" <?php if($part == 'list'){?>class="current"<?php }?>>已添加的展会类别</a></li>
                <li><a title="新增展会类别" href="zhchannel.php?part=add" <?php if($part == 'add'){?>class="current"<?php }?>>新增展会类别</a></li>
            </ul>
        </div>
    </div>
</div>
...
?>
  ...
  <td><li style="margin-left:<?=$cat['level']>1?$cat[level]*3:$cat[level]?>em;" <?php if($cat['parentid'] != '0') echo 'class="son"'?>><a <?php if($cat[level] == 0){?>style="font-weight:bold" <?}?> href="../zhanhui.php?catid=<?=$cat[catid]?>" target="_blank"><?=$cat[catname]?></a></li></td>
      <td width="80"><input name="catorder[<?=$cat[catid]?>]" value="<?=$cat[catorder]?>" class="txt"/></td>
	  <td><a href="zhchannel.php?part=edit&catid=<?=$cat[catid]?>">编辑</a> / <a href="zhchannel.php?part=del&catid=<?=$cat[catid]?>" onClick="if(!confirm('确定要删除该展会栏目吗？\n\n该操作将删除隶属该栏目的子栏目以及展会文章！'))return false;">删除</a></td>
