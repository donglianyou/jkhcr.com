...
case "login" :
				...
				if ( $part == "chk" )
				{
								...
								if ( $row )
								{
												...
												$SystemGlobalcfm_admin->qq3479015851_admin_login( $admin_id, $admin_name, $admin_cityid );
				...
				{
								define( "SysGlbCfm", TRUE );
								$SystemGlobalcfm_admin->qq3479015851_admin_logout( );
...
case "manage" :
				...
				if ( $part == "left" )
				{
								...
								$part = trim( $_GET['part'] );
								$part = $part ? $part : "info";
								$SystemGlobalcfm_admin_menu = qq3479015851_admin_menu( "left" );
				...
				else
				{
								if ( $part == "top" )
								{
												require_once( SysGlbCfm_INC."/db.class.php" );
												require_once( dirname( __FILE__ )."/include/".( $admin_cityid ? "qq3479015851.citymenu.inc.php" : "qq3479015851.menu.inc.php" ) );
												$SystemGlobalcfm_admin_menu = qq3479015851_admin_menu( "top" );
												$admindir = getcwdol( );
								...
								else
								{
												...
												foreach ( $ele as $w => $v )
												{
																$SystemGlobalcfm_count_str .= $w == "siteabout" ? "<div class=\"clear\"></div>" : "";
																$SystemGlobalcfm_count_str .= "<div class=\"countsquare\">\r\n\t\t\t\t<div class=\"ab\">\r\n\t\t\t\t";
																foreach ( $element[$w] as $k => $u )
																{
																				$SystemGlobalcfm_count_str .= "<div class=\"b\">";
																				$SystemGlobalcfm_count_str .= $u[where] ? "<a href=\"#\" onclick=\"parent.framRight.location='".$u['url']."';\">".$k."<br><div class=\"c\">".qq3479015851_count( $u[table], $u[where] )."</div></a>" : "<a href=\"#\" onclick=\"parent.framRight.location='".$u['url']."';\">".$k."<br><div class=\"c\">".qq3479015851_count( $u[table] )."</div></a>";
																				$SystemGlobalcfm_count_str .= "</div>";
																}
																$SystemGlobalcfm_count_str .= "</div>\r\n\t\t\t\t<div class=\"a\">".$v."</div>\r\n\t\t\t\t</div>";
												}
												...
												{
																$SystemGlobalcfm_welcome_str .= "<tr bgcolor=\"#f5fbff\"><td width=\"15\" bgcolor=\"#F6FBFF\" style=\"\">".$k."</td><td bgcolor=\"white\" style=\"padding:15px;\" class=\"other\">".$value."</td></tr>";
												}
												$here = "系统管理首页";
