...
else if ( $action == "delall" )
{
				...
				$db->query( "DELETE FROM `".$db_qq3479015851."comment` WHERE id ".create_in( $id )." AND type = '".$part."'" );
...
else if ( $action == "level0" )
{
				...
				foreach ( $id as $k => $v )
				{
								$db->query( "UPDATE `".$db_qq3479015851."comment` SET comment_level = 0 WHERE id = '".$v."' AND type = '".$part."'" );
				}
...
else if ( $action == "level1" )
{
				...
				foreach ( $id as $k => $v )
				{
								$db->query( "UPDATE `".$db_qq3479015851."comment` SET comment_level = 1 WHERE id = '".$v."' AND type = '".$part."'" );
				}
...
else if ( $action == "list" )
{
				...
				$sql = "SELECT a.id,a.userid,a.content,a.pubtime,a.ip,a.typeid,a.type,b.title,a.comment_level FROM `".$db_qq3479015851."comment` AS a LEFT JOIN `".$db_qq3479015851.$part."` AS b ON a.typeid = b.id ".$where." ORDER BY a.pubtime DESC";
				$rows_num = $db->getOne( "SELECT COUNT(*) FROM `".$db_qq3479015851."comment` AS a ".$where );
