...
case "link" :
				$part = $part ? $part : "list";
				require_once( SysGlbCfm_INC."/brand.fun.php" );
				if ( $part == "list" )
				{
				...
				else if ( $part == "update" )
				{
								...
								$sql = "UPDATE ".$db_qq3479015851."brand SET cityid='".$cityid."',webname='".$webname."',weblogo='".$weblogo."',url='".$url."',catid='".$catid."',ordernumber='".$ordernumber."',createtime='".$timestamp."',ifindex='".$ifindex."',ischeck='".$ischeck."',msg='".textarea_post_change( $msg )."',name='".$name."',qq='".$qq."',email='".$email."',typeid='".$typeid."',dayip='".$dayip."',pr='".$pr."' WHERE id = '".$id."'";
								$res = $db->query( $sql );
								clear_cache_files( "city_".$cityid );
				...
				else
				{
								...
								else
								{
												...
												if ( is_array( $ordernumber ) )
												{
																...
																{
																				$db->query( "UPDATE `".$db_qq3479015851."brand` SET ordernumber = '".$value."' WHERE id = '".$korder."'" );
																}
												}
												if ( is_array( $ids ) && in_array( $do_action, array( "index", "inside", "del", "check2", "check1" ) ) )
												{
																if ( $do_action == "index" )
																{
																				$db->query( "UPDATE `".$db_qq3479015851."brand` SET ifindex = '2' WHERE ".create_in( $ids, "id" ) );
																}
																else if ( $do_action == "inside" )
																{
																				$db->query( "UPDATE `".$db_qq3479015851."brand` SET ifindex = '1' WHERE ".create_in( $ids, "id" ) );
																}
																else if ( $do_action == "del" )
																{
																				$db->query( "DELETE FROM `".$db_qq3479015851."brand` WHERE ".create_in( $ids, "id" ) );
																}
																else if ( $do_action == "check2" )
																{
																				$db->query( "UPDATE `".$db_qq3479015851."brand` SET ischeck = '2' WHERE ".create_in( $ids, "id" ) );
																}
																else if ( $do_action == "check1" )
																{
																				$db->query( "UPDATE `".$db_qq3479015851."brand` SET ischeck = '1' WHERE ".create_in( $ids, "id" ) );
																}
												}
												clear_cache_files( "city_".$cityid );
...
case "type" :
				...
				else if ( $part == "insert" )
				{
								...
								$sql = "Insert Into `".$db_qq3479015851."brand_type`(id,typename)\r\n\t\t\t\tValues('','".$typename."');";
								$res = $db->query( $sql );
				...
				else
				{
								if ( $part == "update" )
								{
												...
												$sql = "UPDATE ".$db_qq3479015851."brand_type SET typename='".$typename."' WHERE id = '".$id."'";
												$res = $db->query( $sql );
