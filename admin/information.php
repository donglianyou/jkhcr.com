...
{
	case "report" :
	require_once( SysGlbCfm_DATA."/report.type.inc.php" );
	if ( $action == "list" )
	{
	...
	else if ( $action == "del" )
	{
		...
		write_msg( "举报记录".$id."已成功删除！", $url, "SysGlbCfm_record" );
	}
	else if ( $action == "delall" )
	{
		write_msg( "举报记录".qq3479015851_del_all( "info_report", $id )."删除成功！", $url, "SysGlbCfm" );
	}
	break;
	default :
	if ( $action == "delall" )
	...
	else if ( $action == "list" )
	{
		...
		foreach ( $page1 as $k => $row )
		{
			...
			$arr['contact_who'] = $row['ismember'] == 1 ? "<a href=\"javascript:void(0);\" onclick=\"setbg('SysGlbCfm会员中心',400,110,'../box.php?part=member&userid=".$row['userid']."&admindir={$admindir}')\">".$row[userid]."</a>" : $row['contact_who'];
			$arr['title'] = $row['title'];
			$arr['catid'] = $row['catid'];
	...
	else if ( $action == "edit" )
	{
		switch ( $do )
		{
			...
			$post['upload_img'] = get_upload_image_edit( $cat['if_upimg'], $id, "SysGlbCfm" );
	...
	else if ( $action == "pm" )
	{
		...
		require_once( SysGlbCfm_DATA."/pm_message.inc.php" );
		$here = "信息操作附加处理";
...
$SystemGlobalcfm_global = $db = $db_qq3479015851 = $part = $idin = $rows_num = $page = $pages_num = $per_page = $per_screen = $startid = NULL;
?>
