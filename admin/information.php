...
{
	case "report" :
	require_once( QQ3479015851_DATA."/report.type.inc.php" );
	if ( $action == "list" )
	{
		...
		$rows_num = qq3479015851_count( "info_report", $where );
		$param = setparam( array( "part", "type" ) );
		$report = array( );
		$page1 = page1( "SELECT * FROM `".$db_qq3479015851."info_report` {$where} ORDER BY pubtime DESC" );
		...
		include( qq3479015851_tpl( "info_report" ) );
	}
	else if ( $action == "del" )
	{
		qq3479015851_delete( "info_report", "WHERE id='".$id."'" );
		write_msg( "举报记录".$id."已成功删除！", $url, "QQ3479015851_record" );
	}
	else if ( $action == "delall" )
	{
		write_msg( "举报记录".qq3479015851_del_all( "info_report", $id )."删除成功！", $url, "QQ3479015851" );
	}
	break;
	default :
	if ( $action == "delall" )
	{
		...
		{
			$get_row = $db->getrow( "SELECT a.*,b.modid FROM `".$db_qq3479015851."information` AS a LEFT JOIN `{$db_qq3479015851}category` AS b ON a.catid = b.catid WHERE a.id = '{$w}'" );
			if ( 1 < $get_row['modid'] )
			{
				qq3479015851_delete( "information_".$get_row[modid], "WHERE id = '".$w."'" );
			}
			if ( empty( $get_row['img_path'] ) )
			{
				$del = $db->getall( "SELECT path,prepath FROM `".$db_qq3479015851."info_img` WHERE infoid='{$w}'" );
				foreach ( $del as $k => $v )
				{
					@unlink( @QQ3479015851_ROOT.@$v[path] );
					@unlink( @QQ3479015851_ROOT.@$v[prepath] );
				}
				qq3479015851_delete( "info_img", "WHERE infoid = '".$w."'" );
			}
			qq3479015851_delete( "comment", "WHERE type = 'information' AND typeid = '".$w."'" );
		...
		if ( qq3479015851_delete( "information", " WHERE id IN (".$id.")" ) )
		{
			write_msg( "分类信息删除成功！", $url, "qq3479015851" );
		}
	}
	else if ( $action == "upgrade_index" )
	{
		...
		{
			case "-1" :
			$db->query( "UPDATE `".$db_qq3479015851."information` SET upgrade_type_index = '1',upgrade_time_index='' WHERE id IN({$id})" );
			$cz = "取消首页置顶";
			break;
			default :
			$upgrade_time = $upgrade_time * 3600 * 24 + $timestamp;
			$db->query( "UPDATE `".$db_qq3479015851."information` SET upgrade_type_index = '2',upgrade_time_index='{$upgrade_time}' WHERE id IN({$id})" );
			$cz = "首页置顶";
			break;
		}
	...
	else if ( $action == "upgrade" )
	{
		...
		{
			case "-1" :
			$db->query( "UPDATE `".$db_qq3479015851."information` SET upgrade_type = '1',upgrade_time='' WHERE id IN({$id})" );
			$cz = "取消大类置顶";
			break;
			default :
			$upgrade_time = $upgrade_time * 3600 * 24 + $timestamp;
			$db->query( "UPDATE `".$db_qq3479015851."information` SET upgrade_type = '2',upgrade_time='{$upgrade_time}' WHERE id IN({$id})" );
			$cz = "大类置顶";
		}
		write_msg( "信息主题批量".$cz."操作成功!", $url, "write_record" );
	}
	else if ( $action == "upgrade_list" )
	{
		...
		{
			case "-1" :
			$db->query( "UPDATE `".$db_qq3479015851."information` SET upgrade_type_list = '1',upgrade_time_list='' WHERE id IN({$id})" );
			$cz = "取消小类置顶";
			break;
			default :
			$upgrade_time = $upgrade_time * 3600 * 24 + $timestamp;
			$db->query( "UPDATE `".$db_qq3479015851."information` SET upgrade_type_list = '2',upgrade_time_list='{$upgrade_time}' WHERE id IN({$id})" );
			$cz = "小类置顶";
		}
		write_msg( "信息主题批量".$cz."操作成功!", $url, "write_record" );
	}
	else if ( $action == "ifred" )
	{
		...
		$db->query( "UPDATE `".$db_qq3479015851."information` {$set} WHERE id IN({$id})" );
	...
	else if ( $action == "certify_yes" )
	{
		...
		$db->query( "UPDATE `".$db_qq3479015851."information` SET certify = '1' WHERE id IN({$id})" );
		write_msg( "操作成功！", $url, "write_record" );
	}
	else if ( $action == "certify_no" )
	{
		...
		$db->query( "UPDATE `".$db_qq3479015851."information` SET certify = '0' WHERE id IN({$id})" );
		write_msg( "操作成功！", $url, "write_record" );
	}
	...
	else if ( $action == "list" )
	{
		...
		$information = array( );
		$idin = get_page_idin( "id", "SELECT a.id FROM `".$db_qq3479015851."information` AS a {$where} ORDER BY a.id DESC" );
		$page1 = $idin ? $db->getall( "SELECT a.* FROM `".$db_qq3479015851."information` AS a WHERE a.id in ({$idin}) ORDER BY a.id DESC" ) : array( );
		foreach ( $page1 as $k => $row )
		{
			$arr['id'] = $row['id'];
			...
			$arr['contact_who'] = $row['ismember'] == 1 ? "<a href=\"javascript:void(0);\" onclick=\"setbg('QQ3479015851会员中心',400,110,'../box.php?part=member&userid=".$row['userid']."&admindir={$admindir}')\">".$row[userid]."</a>" : $row['contact_who'];
			$arr['title'] = $row['title'];
			$arr['catid'] = $row['catid'];
		...
		include( qq3479015851_tpl( "information_list" ) );
		$idin = NULL;
	}
	else if ( $action == "edit" )
	{
		switch ( $do )
		{
			...
			qq3479015851_check_upimage( "qq3479015851_img_" );
			if ( empty( $cityid ) )
			{
				write_msg( "请选择发布所在分站！" );
			}
			...
			if ( is_array( $extra ) )
			{
				...
				if ( 1 < $modid )
				{
					...
					if ( $sql )
					{
						$db->query( "UPDATE `".$db_qq3479015851."information_{$modid}` SET {$sql} WHERE id = '{$id}'" );
						$sql = NULL;
					}
				}
			}
			...
			$catid = $post['catid'];
			$areaid = $post['areaid'];
			$cat = $db->getrow( "SELECT a.if_upimg,a.if_mappoint,a.modid,b.catid FROM `".$db_qq3479015851."category` AS a LEFT JOIN `{$db_qq3479015851}category` AS b ON a.parentid = b.catid WHERE a.catid = '{$catid}'" );
			...
			$post['qq3479015851_extra_value'] = return_category_info_options( $cat['modid'], $id );
			$post['upload_img'] = get_upload_image_edit( $cat['if_upimg'], $id, "QQ3479015851" );
			...
			$post['submit'] = "修改";
			$here = "信息内容修改";
			include( qq3479015851_tpl( "information_edit" ) );
			break;
		}
	}
	else if ( $action == "pm" )
	{
		...
		require_once( QQ3479015851_DATA."/pm_message.inc.php" );
		$here = "信息操作附加处理";
	...
	else if ( strstr( $action, "level" ) )
	{
		...
		foreach ( $id as $k => $v )
		{
			...
			$db->query( "UPDATE `".$db_qq3479015851."information` SET info_level = '{$action}' WHERE id = '{$v}'" );
		}
...
$qq3479015851_global = $db = $db_qq3479015851 = $part = $idin = $rows_num = $page = $pages_num = $per_page = $per_screen = $startid = NULL;
?>
