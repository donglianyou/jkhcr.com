...
switch ($do){
	...
	if($part == 'default'){
		...
		if($do_action == 'delall') {
			...
			foreach ($id as $k => $v){
				...
				/*删除附表信息*/
				$query = $db -> query("SELECT a.id,b.modid FROM `{$db_qq3479015851}information` AS a LEFT JOIN `{$db_qq3479015851}category` AS b ON a.catid = b.catid WHERE a.userid = '$row[userid]'");
				while($r = $db -> fetchRow($query)){
					if($r[modid] > 1) qq3479015851_delete("information_".$r[modid],"WHERE id = '$r[id]'");
				}
				
				
				$row['userid'] && qq3479015851_delete("information","WHERE userid = '$row[userid]'");
				$row['userid'] && qq3479015851_delete("member_docu","WHERE userid = '$row[userid]'");
				...
				$row['userid'] && qq3479015851_delete("member_record_login","WHERE userid = '$row[userid]'");
				$row['userid'] && qq3479015851_delete("member_record_use","WHERE userid = '$row[userid]'");
				$row['userid'] && qq3479015851_delete("member_comment","WHERE userid = '$row[userid]'");
				
				/*删除插件里的信息*/
				$row['userid'] && qq3479015851_delete("goods","WHERE userid = '$row[userid]'");
				$row['userid'] && qq3479015851_delete("coupon","WHERE userid = '$row[userid]'");
				$row['userid'] && qq3479015851_delete("group","WHERE userid = '$row[userid]'");
				
			}
			
			write_msg('成功删除编号为'.qq3479015851_del_all("member",$id).'的会员',$url,"qq3479015851");
			...
			foreach ($_POST['id'] as $k => $v){
				$row = $db->getRow("SELECT userid FROM `{$db_qq3479015851}member` WHERE id ='$v'");
			...
			write_msg('成功删除会员发布的分类信息',$url,"qq3479015851");
			...
			foreach ($_POST['id'] as $k => $v){
				$row = $db->getRow("SELECT id,userid FROM `{$db_qq3479015851}member` WHERE id ='$v'");
				$row['userid'] && qq3479015851_delete("member_docu","WHERE userid = '$row[userid]'");
			}
			write_msg('成功删除会员发布的空间文档信息',$url,"qq3479015851");
			...
			write_msg('成功删除会员的网友点评信息',$url,"qq3479015851");
			...
			foreach ($_POST['id'] as $k => $v){
				$row = $db->getRow("SELECT userid FROM `{$db_qq3479015851}member` WHERE id ='$v'");
				$row['userid'] && qq3479015851_delete("member_pm","WHERE fromuser = '$row[userid]' OR fromuser = '$row[userid]'");
			}
			
			write_msg('成功删除会员的短消息记录',$url,"qq3479015851");
			...
			write_msg('成功删除会员的空间相册信息',$url,"qq3479015851");
		} elseif($do_action == 'person') {
			
			empty($id) && write_msg("请选择指定会员");
			
			foreach ($_POST['id'] as $k => $v){
				$db -> query("UPDATE `{$db_qq3479015851}member` SET if_corp = '0',tname='' WHERE id = '$v'");	
			}
			...
			foreach ($_POST['id'] as $k => $v){
				$db -> query("UPDATE `{$db_qq3479015851}member` SET per_certify = '1' WHERE id = '$v'");	
			}
			...
			foreach ($_POST['id'] as $k => $v){
				$db -> query("UPDATE `{$db_qq3479015851}member` SET com_certify = '1' WHERE id = '$v'");	
			}
			...
			foreach ($_POST['id'] as $k => $v){
				$db -> query("UPDATE `{$db_qq3479015851}member` SET if_corp = '1' WHERE id = '$v'");	
			}
			...
			is_array($id) && $db -> query("UPDATE `{$db_qq3479015851}member` SET ifindex = '2' WHERE ".create_in($id,'id'));
			...
			is_array($id) && $db -> query("UPDATE `{$db_qq3479015851}member` SET ifindex = '1' WHERE ".create_in($id,'id'));
			write_msg("您的处理请求已经提交成功！",$url);
			...
			is_array($id) && $db -> query("UPDATE `{$db_qq3479015851}member` SET iflist = '2' WHERE ".create_in($id,'id'));
			write_msg("您的处理请求已经提交成功！",$url);
			...
			is_array($id) && $db -> query("UPDATE `{$db_qq3479015851}member` SET iflist = '1' WHERE ".create_in($id,'id'));
			write_msg("您的处理请求已经提交成功！",$url);
			
		} elseif(is_numeric($do_action)) {
			
			$nowval = $db -> getOne("SELECT levelname FROM `{$db_qq3479015851}member_level` WHERE id = '$do_action'");
			...
			if(is_array($_POST['id'])){
				foreach ($_POST['id'] as $k => $v){
					$db -> query("UPDATE `{$db_qq3479015851}member` SET levelid = '$do_action' WHERE id = '$v'");	
				}
			...
			$sql = ($catid != '' && $if_corp == 1) ? "SELECT a.id,a.if_corp,a.money_own,a.userid,a.userpwd,a.joinip,a.logintime,a.credit,a.credits,a.jointime,a.levelid,b.levelname,a.tname,a.levelup_time,a.ifindex,a.iflist FROM `{$db_qq3479015851}member` AS a LEFT JOIN `{$db_qq3479015851}member_category` AS f ON a.userid = f.userid LEFT JOIN `{$db_qq3479015851}member_level` AS b ON a.levelid = b.id $where ORDER BY a.id DESC" : "SELECT a.id,a.if_corp,a.money_own,a.userid,a.userpwd,a.joinip,a.logintime,a.credit,a.credits,a.jointime,a.levelid,b.levelname,a.tname,a.levelup_time,a.ifindex,a.iflist FROM {$db_qq3479015851}member AS a LEFT JOIN `{$db_qq3479015851}member_level` AS b ON a.levelid = b.id  $where ORDER BY a.id DESC";
			...
			$rows_num = $db->getOne(($catid != '' && $if_corp == 1) ?  "SELECT COUNT(*) FROM `{$db_qq3479015851}member` AS a LEFT JOIN `{$db_qq3479015851}member_category` AS f ON a.userid = f.userid $where" : "SELECT COUNT(*) FROM `{$db_qq3479015851}member` AS a $where");
			$member = page1($sql);
			...
			include(qq3479015851_tpl("member_default"));
		...
		$here = "新增会员";
		include(qq3479015851_tpl($if_corp == 1 ? 'member_shop' : 'member'));
		...
		if(PASSPORT_TYPE == 'phpwind'){
			...
			if(!empty($activation) && ($activeuser = uc_get_user($activation))) {
				...
				if(uc_get_user($userid) && !$db->getOne("SELECT userid FROM {$db_qq3479015851}member WHERE userid='$userid'")) {
		...
		if($userid) {
			...
			switch ($reg_corp){
				case '0':
				$db->query("UPDATE `{$db_qq3479015851}member` SET cname = '$cname',levelid = '$levelid',money_own='$money_own',if_corp = '0',status='1' WHERE userid = '$userid'");
				break;
				...
				$db->query("UPDATE `{$db_qq3479015851}member` SET levelid='$levelid',tname = '$tname',cname = '$cname',catid = '$catids', cityid = '$cityid',areaid='$areaid',streetid='$streetid',qq='$qq',msn='$msn',email='$email',address='$address',busway='$busway',money_own='$money_own',score='$score',credit='$credit',credits='$credits',tel='$tel',mobile='$mobile',mappoint='$mappoint',introduce='$content',web='$web',if_corp='1',template='$template',status='1' WHERE userid = '$userid'");
				if(is_array($catid)){
					foreach($catid as $kids => $vids){
						$db->query("INSERT `{$db_qq3479015851}member_category` (userid,catid)VALUES('$userid','$vids')");
					}
				}
				break;
			}
			
			write_msg("添加会员 <b>".$userid."</b> 成功","member.php","qq3479015851");
		...
		$id = intval( $id );
		if ( !( $row = $db->getRow( "SELECT userid,if_corp,tname,money_own FROM `".$db_qq3479015851."member` WHERE id = '".$id."'" ) ) )
		{
						write_msg( "您充值的会员不存在！" );
		}
		$list = $db->getAll( "SELECT * FROM `".$db_qq3479015851."payrecord` WHERE userid = '".$row['userid']."' ORDER BY id DESC" );
		$here = "充值金币";
		include( qq3479015851_tpl( "member_pay" ) );
		
	}elseif($part == 'payupdate'){
		...
		$paymoney = floor( $paymoney );
			...
			$db->query( "UPDATE `".$db_qq3479015851."member` SET money_own = money_own + ".$paymoney." WHERE id  = '".$id."'" );
			$payip = getip( );
			$db->query( "INSERT INTO ".$db_qq3479015851."payrecord(id,uid,userid,orderid,money,posttime,paybz,type,payip) values('','".$id."','".$userid."','".$timestamp."','".$paymoney."','".$timestamp."','充值成功','管理员".$_obfuscate_YWRtaW5faWTlhYXlgLw."','".$payip."');" );
			write_msg( $userid.":成功充值 ".$paymoney." 金币！", "?part=pay&id=".$id );
		
	}elseif($part == 'edit'){
		
		$sql = "SELECT a.*,b.id as levelid,b.levelname FROM `{$db_qq3479015851}member` AS a LEFT JOIN `{$db_qq3479015851}member_level` AS b ON a.levelid = b.id WHERE a.id = '$id'";
		$edit = $db->getRow($sql);
		...
		if(PASSPORT_TYPE == 'phpwind'){
			...
			$old=$db->getRow("SELECT id,userid FROM `{$db_qq3479015851}member` WHERE id = '$id'");
			if($db->getOne("SELECT id FROM `{$db_qq3479015851}member` WHERE userid LIKE '$userid' AND userid != '$old[userid]'")){
		...
		if($reg_corp == '1'){
			if(is_array($catid)){
				qq3479015851_delete("member_category","WHERE userid = '$userid'");
				foreach($catid as $kids => $vids){
					$db->query("INSERT `{$db_qq3479015851}member_category` (userid,catid)VALUES('$userid','$vids')");
				}
				$catid  = implode(',',$catid);
		...
		$sql = ($if_corp == '1' && $reg_corp == '1') ? "UPDATE `{$db_qq3479015851}member` SET {$userpwd} userid = '$userid', levelid='$levelid',tname = '$tname',cname = '$cname',catid = '$catid', cityid = '$cityid',areaid = '$areaid',streetid = '$streetid',qq='$qq',msn='$msn',email='$email',address='$address',busway='$busway',money_own='$money_own',tel='$tel',mobile='$mobile',mappoint='$mappoint',introduce='$content',web='$web',per_certify='$per_certify',com_certify='$com_certify',score='$score',credit='$credit',credits='$credits',template='$template' WHERE id = '$id'" : "UPDATE `{$db_qq3479015851}member` SET {$userpwd} userid = '$userid', levelid='$levelid',cname = '$cname',email='$email',money_own='$money_own',per_certify='$per_certify',com_certify='$com_certify',score='$score',credit='$credit',credits='$credits' WHERE id = '$id'";
		...
		qq3479015851_delete("config","WHERE description = 'levelup_notice'");
		$db -> query("INSERT INTO `{$db_qq3479015851}config` SET value = '$levelup_notice', type = 'levelup', description = 'levelup_notice'");
		...
		if($do_action == 'del'){
			...
			if($do_act == 'allperson'){
					//删除所有待审个人会员
				$db -> query("DELETE FROM `{$db_qq3479015851}member` WHERE if_corp = '0' AND status = '0'");
				$url = 'member.php?part=verify&do_action=default&type=person';
			} elseif($do_act == 'allstore'){
					//删除所有待审商家会员
				$id = $db ->getAll("SELECT id FROM `{$db_qq3479015851}member` WHERE if_corp = '1' AND status = '0'");
				...
				$db -> query("DELETE FROM `{$db_qq3479015851}member` WHERE if_corp = '1' AND status = '0'");
				$url = 'member.php?part=verify&do_action=default&type=store';
			} elseif($do_act == 'person'){
					//删除指定个人会员
				empty($id) && write_msg('请选择对象!');
				$ids = create_in($id,'id');
				$db -> query("DELETE FROM `{$db_qq3479015851}member` WHERE if_corp = '0' AND status = '0' AND $ids");
				...
				$ids = create_in($id,'id');
				$db -> query("DELETE FROM `{$db_qq3479015851}member` WHERE if_corp = '1' AND status = '0' AND $ids");
			}
			...
			if($do_act == 'allperson'){
					//通过所有待审个人会员
				$db->query("UPDATE `{$db_qq3479015851}member` SET status = '1' WHERE status = '0' AND if_corp = '0'");
				$url = 'member.php?part=verify&do_action=default&type=person';
			} elseif($do_act == 'allstore'){
					//通过所有待审商家会员
				$db->query("UPDATE `{$db_qq3479015851}member` SET status = '1' WHERE status = '0' AND if_corp = '1'");
				$url = 'member.php?part=verify&do_action=default&type=store';
			} elseif($do_act == 'person'){
					//通过指定个人会员
				empty($id) && write_msg('请选择对象!');
				$ids = create_in($id,'id');
				$db->query("UPDATE `{$db_qq3479015851}member` SET status = '1' WHERE status = '0' AND if_corp = '0' AND $ids");
			} elseif($do_act == 'store'){
					//通过指定商家会员
				empty($id) && write_msg('请选择对象!');
				$ids = create_in($id,'id');
				$db->query("UPDATE `{$db_qq3479015851}member` SET status = '1' WHERE status = '0' AND if_corp = '1' AND $ids");
			}
			...
			if($do_act == 'allstore'){
					//否决所有商家会员
				$db->query("UPDATE `{$db_qq3479015851}member` SET status = '1',if_corp = '0' WHERE status = '0' AND if_corp = '1'");
				$url = 'member.php?part=verify&do_action=default&type=store';
			} elseif($do_act == 'store'){
					//否决指定商家会员
				empty($id) && write_msg('请选择对象!');
				$ids = create_in($id,'id');
				$db->query("UPDATE `{$db_qq3479015851}member` SET status = '1',if_corp = '0' WHERE status = '0' AND if_corp = '1' AND $ids");
			}
			...
			$count['person'] = qq3479015851_count("member",$where1);
			$count['store'] = qq3479015851_count("member",$where2);
			...
			if($type=='all') $where .="";
			$sql = "SELECT * FROM {$db_qq3479015851}member $where ORDER BY id DESC";
			$param = setParam(array('do','part','type','do_action'));
			$rows_num = qq3479015851_count("member",$where);
			$member = page1($sql);
			$here="待审用户列表";
			include qq3479015851_tpl('member_verify');
		} else{
	...
	if ($part == 'list'){
		...
		$sql = "SELECT * FROM {$db_qq3479015851}member_level ORDER BY id desc";
		$group = $db->getAll($sql);
		$here = "注册用户组管理";
		...
		include(qq3479015851_tpl("member_group"));	
		
	}elseif($part == 'add'){
		
		chk_admin_purview("purview_会员组");
		$here = "新增用户组";
		include(qq3479015851_tpl("member_group_add"));
		...
		if(!empty($levelname)){
			if($db->getOne("select count(*) from {$db_qq3479015851}member_level where levelname = '$levelname'")){
		...
		$group = $db->getRow("SELECT * FROM `{$db_qq3479015851}member_level` WHERE id = '$id'");
		...
		$here = "设置用户组权限";
		include(qq3479015851_tpl("member_group_edit"));
		...
		if(!$settings['ifopen']) write_msg('您至少要选择启用一个升级期限');
		$settings = serialize($settings);
		$db->query("UPDATE `{$db_qq3479015851}member_level` SET levelname='$levelname',purviews='$purview',money_own='$money_own',perday_maxpost='$perday_maxpost',signin_view='$signin_view',signin_del='$signin_del',signin_notice='$signin_notice',member_contact='$member_contact',allow_tpl='$allow_tpl',moneysettings = '$settings' WHERE id = '$id'");
		clear_cache_files('member_'.$id);
		write_msg("用户组 ".$levelname." 权限设置成功","member.php?do=group&part=edit&id=".$id,"qq3479015851");
		
	}elseif($part == 'delete'){
		
		if(empty($id)){
			write_msg("没有选择记录");
		}elseif (qq3479015851_count("member","WHERE levelid = '$id'")>0){
			write_msg("该用户组下尚有成员，不能删除！");
		}else{
			qq3479015851_delete("member_level","WHERE id = '$id'");
			clear_cache_files('member_'.$id);
