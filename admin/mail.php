...
{	
	switch ( $part )
	{
		...
		$here = "邮件服务器设置";
		$res = $db->query( "SELECT description,value FROM ".$db_qq3479015851."config WHERE type='mail'" );
		...
		$here = "邮件发送记录";
		$list = $db->getall( "SELECT * FROM `".$db_qq3479015851."mail_sendlist` ORDER BY last_send DESC" );
		break;
		default :
		...
		if ( $template_id )
		{
			$edit = $db->getrow( "SELECT * FROM `".$db_qq3479015851."mail_template` WHERE template_id = '{$template_id}'" );
		}
		break;
	}
...
else
{
	...
	if ( $part == "setting" )
	{
		...
		qq3479015851_delete( "config", "WHERE type = 'mail'" );
		foreach ( $des as $key )
		{
			$db->query( "INSERT ".$db_qq3479015851."config (description,value,type) VALUES ('{$key}','".trim( $$key )."','mail')" );
		}
	...
	else if ( $part == "template" )
	{
		...
		if ( is_array( $delids ) )
		{
			...
			{
				qq3479015851_delete( "mail_template", "WHERE template_id = ".$vids );
			}
		}
		if ( is_array( $add ) && $add[template_subject] && $add[template_code] )
		{
			if ( $db->getone( "SELECT template_id FROM `".$db_qq3479015851."mail_template` WHERE template_code = '{$add['template_code']}'" ) )
			{
				write_msg( "您填写的模板标识码重复了！" );
			}
			else
			{
				$db->query( "INSERT `".$db_qq3479015851."mail_template` (template_subject,template_code,is_html,is_sys,last_modify)VALUES('{$add['template_subject']}','{$add['template_code']}','{$add['is_html']}','{$add['is_sys']}','".time( )."')" );
			}
		}
		if ( is_array( $edit ) && $edit[template_id] && $edit[template_subject] && $edit[template_code] && $edit[template_content] )
		{
			$db->query( "UPDATE `".$db_qq3479015851."mail_template` SET template_subject = '{$edit['template_subject']}', template_code = '{$edit['template_code']}' , is_html  = '{$edit['is_html']}' , template_content = '{$edit['template_content']}' , last_modify = '".time( ).( "'  WHERE template_id = '".$edit['template_id']."'" ) );
	...
	else if ( $part == "sendlist" )
	{
		...
		if ( is_array( $delids ) )
		{
			foreach ( $delids as $kids => $vids )
			{
				qq3479015851_delete( "mail_sendlist", "WHERE id = ".$vids );
			}
		}
