...
if ( !submit_check( CURSCRIPT."_submit" ) )
{
				...
				{
								$here = "广告修改";
								if ( !( $edit = $db->getRow( "SELECT * FROM `".$db_qq3479015851."advertisement` WHERE advid = ".$advid ) ) )
								{
												write_msg( "当前广告不存在！" );
								}
				...
				else
				{
								...
								$rows_num = qq3479015851_count( CURSCRIPT, "WHERE 1".$where );
								$param = setparam( array( "part", "type", "cityid" ) );
								$adv = array( );
								//echo "SELECT * FROM `".$db_qq3479015851."advertisement` WHERE 1 ".$where." ORDER BY displayorder ASC";
								$adv = page1( "SELECT * FROM `".$db_qq3479015851."advertisement` WHERE 1 ".$where." ORDER BY displayorder ASC" );
				}
				include( $part == "add" || $part == "edit" ? qq3479015851_tpl( CURSCRIPT ) : qq3479015851_tpl( CURSCRIPT."_".$part ) );
}
else
{
				if ( $part == "list" )
				{
								if ( is_array( $titlenew ) )
								{
												...
												{
																$db->query( "UPDATE `".$db_qq3479015851."advertisement` SET title = '".$value."',displayorder='".$displayorder[$key]."',available='".$available[$key]."' WHERE advid = '".$key."'" );
												}
								}
								if ( is_array( $delids ) )
								{
												...
												{
																qq3479015851_delete( "advertisement", "WHERE advid = ".$vids );
												}
								}
				}
				else
				{
								...
								if ( $part == "advadd" )
								{
												$db->query( "INSERT INTO ".$db_qq3479015851."advertisement (available, type)\r\n\t\t\t\tVALUES ('1', '".$type."')" );
												$advid = $db->insert_id( );
								}
								else
								{
												$type = $db->getOne( "SELECT type FROM ".$db_qq3479015851."advertisement WHERE advid='".$advid."'" );
								}
								...
								$query = $db->query( "UPDATE ".$db_qq3479015851."advertisement SET title='".$advnew['title']."', targets='".$advnew['targets']."', parameters='".$advnew['parameters']."', code='".$advnew['code']."', starttime='".$advnew['starttime']."', endtime='".$advnew['endtime']."', cityid='".$advnew['cityid']."' WHERE advid='".$advid."'" );
				}
