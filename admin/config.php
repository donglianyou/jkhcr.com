...
else if ( $part == "update" )
{
				...
				foreach ( $admin_global as $k => $a )
				{
								...
								$db->query( "INSERT INTO `".$db_qq3479015851."config` (value,description) VALUES ('".$info_k."','".$k."')" );
				}
				update_config_cache( );
				unset( $admin_global );
...
else if ( $part == "badwords" )
{
				...
				if ( trim( $action ) == "dopost" )
				{
								...
								qq3479015851_delete( "badwords" );
								$db->query( "INSERT INTO `".$db_qq3479015851."badwords` (words,view,ifcheck) VALUES ('".$cfg_badwords0."','".$cfg_badwords1."','".$cfg_badwords2."')" );
				...
				{
								$here = "违禁词语过滤设置";
								$res = $db->query( "SELECT words,view,ifcheck FROM ".$db_qq3479015851."badwords" );
								while ( $row = $db->fetchRow( $res ) )
								{
...
else if ( $part == "commentsettings" )
{
				if ( $action == "do_post" )
				{
								$db->query( "DELETE FROM `".$db_qq3479015851."config` WHERE description = 'comment' AND type = 'comment'" );
								$db->query( "INSERT INTO `".$db_qq3479015851."config` (description,value,type) values ('comment','".serialize( $comment )."','comment')" );
				...
				{
								$here = "网友评论点评设置";
								$comment = $db->getOne( "SELECT value FROM `".$db_qq3479015851."config` WHERE type='comment' AND description = 'comment'" );
...
else if ( $part == "cache_sys" )
{
				...
				include( qq3479015851_tpl( "cache_sys" ) );
}
...
else if ( $part == "cache" )
{
				...
				$here = "页面缓存";
				include( qq3479015851_tpl( "cache" ) );
}
else if ( $part == "cacheupdate" )
{
				qq3479015851_delete( "cache" );
				...
				foreach ( $admin_cache as $q => $w )
				{
								foreach ( $w as $k => $a )
								{
												...
												$db->query( "INSERT INTO `".$db_qq3479015851."cache` (page,time,open) VALUES ('".$cpage."','".$ctime."','".$copen."')" );
								}
				}
...
else if ( $part == "imgcode" )
{
				...
				{
								$db->query( "DELETE FROM `".$db_qq3479015851."config` WHERE type = 'authcode'" );
								if ( is_array( $settingsnew['open'] ) )
								{
												foreach ( $settingsnew['open'] as $key => $val )
												{
																$db->query( "INSERT INTO `".$db_qq3479015851."config` (description,value,type) values ('".$key."','".$val."','authcode')" );
												}
								}
								$settingsnew['type'] = $settingsnew['type'] != "" ? $settingsnew['type'] : "engber";
								$db->query( "INSERT INTO `".$db_qq3479015851."config` (description,value,type) values ('type','".$settingsnew['type']."','authcode')" );
								foreach ( array( "noise", "line", "snow", "distort", "incline", "close", "number" ) as $key )
								{
												$db->query( "INSERT INTO `".$db_qq3479015851."config` (description,value,type) values ('".$key."','".$settingsnew[$key]."','authcode')" );
								}
								write_authcode_cache( );
				...
				else
				{
								...
								include( qq3479015851_tpl( "qq3479015851_".$part ) );
				}
}
else if ( $part == "checkask" )
{
				...
				if ( trim( $action ) == "do_post" )
				{
								qq3479015851_delete( "config", "WHERE type = 'checkanswe'" );
								$db->query( "INSERT INTO `".$db_qq3479015851."config` (description,value,type)VALUES('whenregister','".$whenregister."','checkanswe')" );
								$db->query( "INSERT INTO `".$db_qq3479015851."config` (description,value,type)VALUES('whenpost','".$whenpost."','checkanswe')" );
								if ( is_array( $question ) )
								{
												foreach ( $question as $key => $q )
												{
																...
																if ( !$q && !$a )
																{
																				$db->query( "UPDATE `".$db_qq3479015851."checkanswer` SET question='".$q."', answer='".$a."' WHERE id='".$key."'" );
																}
												}
								}
								if ( is_array( $newquestion ) && is_array( $newanswer ) )
								{
												foreach ( $newquestion as $key => $q )
												{
																...
																if ( !$q && !$a )
																{
																				$db->query( "INSERT INTO\t`".$db_qq3479015851."checkanswer` (question, answer) VALUES ('".$q."', '".$a."')" );
																}
												}
								}
								if ( is_array( $delete ) )
								{
												$db->query( "DELETE FROM `".$db_qq3479015851."checkanswer` WHERE ".create_in( $delete, "id" ) );
								}
								write_checkanswer_cache( );
								update_checkanswer_settings( );
				...
				{
								$here = "验证问答设置";
								$c = $db->getAll( "SELECT * FROM `".$db_qq3479015851."checkanswer` ORDER BY id DESC" );
								$res = $db->query( "SELECT description,value FROM ".$db_qq3479015851."config WHERE type = 'checkanswe'" );
								while ( $row = $db->fetchRow( $res ) )
								{
