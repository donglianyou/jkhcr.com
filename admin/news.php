...
if ( !submit_check( CURSCRIPT."_submit" ) )
{
				...
				if ( $part == "list" )
				{
								...
								$sql = "SELECT a.*,b.catname,c.cityname FROM `".$db_qq3479015851."news` AS a LEFT JOIN `{$db_qq3479015851}channel` AS b ON a.catid = b.catid LEFT JOIN `{$db_qq3479015851}city` AS c ON a.cityid=c.cityid {$where} ORDER BY a.id DESC";
								$rows_num = $db->getOne( "SELECT COUNT(*) FROM `".$db_qq3479015851."news` AS a {$where}" );
				...
				{
								$row = $db->getRow( "SELECT * FROM ".$db_qq3479015851."news WHERE id = '{$id}'" );
...
else if ( $part == "list" )
{
				...
				if ( is_array( $delids ) )
				{
								...
								{
												qq3479015851_delete( "news", "WHERE id = ".$vids );
								}
				}
				else
				{
...
else if ( $part == "add" )
{
				...
				else
				{
								...
								$do_qq3479015851 = $db->query( "INSERT INTO `".$db_qq3479015851."news` (title,cityid,keywords,catid,isbold,iscommend,content,hit,perhit,begintime,introduction,author,source,imgpath) VALUES\r\n('{$title}','{$cityid}','{$keywords}','{$catid}','{$isbold}','{$iscommend}','{$content}','{$hit}','{$perhit}','{$timestamp}','{$introduction}','{$author}','{$from}','{$imgpath}')" );
				}
				$id = $db->insert_id( );
