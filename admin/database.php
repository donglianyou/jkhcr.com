...
if ( $part == "backup" )
{
	...
	else
	{
		...
		if ( $type == "qq3479015851" )
		{
			$tables = arraykeys2( fetchtablelist( $db_qq3479015851 ), "Name" );
		}
		else if ( $type == "custom" )
		{
			$tables = array( );
			if ( empty( $setup ) )
			{
				if ( $tables = $db->fetch_first( "SELECT value FROM `".$db_qq3479015851."config` WHERE description='custombackup'" ) )
				{
			...
			else
			{
				...
				$db->query( "DELETE FROM `".$db_qq3479015851."config` WHERE description = 'custombackup' AND type = 'database'" );
				$db->query( "INSERT INTO `".$db_qq3479015851."config` (description, value, type) VALUES ('custombackup', '{$customtablesnew}','database')" );
				$tables =& $customtables;
			}
		...
		else
		{
			...
			$db->query( "DELETE FROM `".$db_qq3479015851."config` WHERE description = 'latestbackup' AND type = 'database'" );
			$db->query( "INSERT INTO `".$db_qq3479015851."config` (description, value, type) VALUES ('latestbackup', '{$timestamp}','database')" );
...
else if ( $part == "optimize" )
{
	...
	include( !submit_check( "optimize_submit" ) ? qq3479015851_tpl( "db_".$part ) : qq3479015851_tpl( "db_result" ) );
}
else if ( $part == "check" )
{
	chk_admin_purview( "purview_数据库维护" );
	...
	include( !submit_check( "check_submit" ) ? qq3479015851_tpl( "db_".$part ) : qq3479015851_tpl( "db_result" ) );
}
else if ( $part == "repair" )
{
	chk_admin_purview( "purview_数据库维护" );
	...
	include( !submit_check( "repair_submit" ) ? qq3479015851_tpl( "db_".$part ) : qq3479015851_tpl( "db_result" ) );
}
if ( is_object( $db ) )
{
	$db->close( );
