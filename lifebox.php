...
if($part == 'rand' || empty($id)){
	$lifebox = $db -> getRow("SELECT * FROM `{$db_qq3479015851}lifebox` WHERE if_view = '2' ORDER BY rand() LIMIT 0,1");
	$nextlifebox = $db -> getRow("SELECT * FROM `{$db_qq3479015851}lifebox` WHERE id > '$lifebox[id]' AND if_view = '2'");
} elseif($lifebox = $db -> getRow("SELECT * FROM `{$db_qq3479015851}lifebox` WHERE id = '$id' AND if_view = '2'")) {
	$lifebox['typeid'] == '1' && write_msg('',$lifebox['lifeurl']);
	$nextlifebox = $db -> getRow("SELECT * FROM `{$db_qq3479015851}lifebox` WHERE id > '$id' AND if_view = '2'");
}else {
