...
/*焦点图*/
$focus = qq3479015851_get_focus('index',3,$cityid);
if(!$cityid || in_array('focus',$independency)){
	$focus = $focus ? $focus : qq3479015851_get_focus('index',3,NULL);
}

/*网站公告*/
$announce = qq3479015851_get_announce($tpl_index['announce'],$cityid);
if(!$cityid || in_array('announce',$independency)){
	$announce = $announce ? $announce : qq3479015851_get_announce($tpl_index['announce'],NULL);
}

/*便民电话*/
$telephone = qq3479015851_get_telephone($tpl_index[telephone],$cityid);
if(!$cityid || in_array('telephone',$independency)){
	$telephone = $telephone ? $telephone : qq3479015851_get_telephone($tpl_index['telephone'],NULL);
}

/*生活百宝箱*/
$lifebox = qq3479015851_get_lifebox($tpl_index[lifebox],$cityid);
if(!$cityid || in_array('lifebox',$independency)){
	$lifebox = $lifebox ? $lifebox : qq3479015851_get_lifebox($tpl_index['lifebox'],NULL);
}
...
if($tpl_index['banmian'] == 'portal'){
	...
	if($query = $db->query("SELECT catid,dir_typename FROM `{$db_qq3479015851}category` WHERE catid IN($in)")){
	...
	$arr = '';
	$zhaopin = array();
	$query = $db -> query("SELECT a.cityid,a.id,a.title,a.ifred,a.ifbold,a.dir_typename,g.{$tpl_index[portali][company]} FROM `{$db_qq3479015851}information` AS a LEFT JOIN `{$db_qq3479015851}information_{$tpl_index[portal][zhaopinmod]}` AS g ON a.id = g.id WHERE ".get_children($tpl_index[portal][zhaopin])." AND (a.info_level = 1 OR a.info_level = 2) {$city_limit} ORDER BY a.begintime DESC LIMIT 0,12");
	...
	$arr = '';
	$jianli = array();
	$query = $db -> query("SELECT a.id,a.title,a.ifred,a.ifbold,a.contact_who,a.dir_typename,a.cityid FROM `{$db_qq3479015851}information` AS a WHERE ".get_children($tpl_index[portal][jianli])." AND (a.info_level = 1 OR a.info_level = 2) {$city_limit} ORDER BY a.begintime DESC LIMIT 0,4");
	if($query){
		while($row = $db -> fetchRow($query)){
			$arr['id']        = $row['id'];
			$arr['title']     = $row['title'];
	...
	$arr = '';
	$esf = array();
	$query = $db -> query("SELECT a.cityid,a.id,a.title,a.ifred,a.ifbold,a.img_path,a.dir_typename,g.{$tpl_index[portali][acreage]},g.{$tpl_index[portali][prices]} FROM `{$db_qq3479015851}information` AS a LEFT JOIN `{$db_qq3479015851}information_{$tpl_index[portal][ershoufangmod]}` AS g ON a.id = g.id WHERE ".get_children($tpl_index[portal][ershoufang])." AND (a.info_level = 1 OR a.info_level = 2) {$city_limit} ORDER BY a.begintime DESC LIMIT 0,12");
	while($row = $db -> fetchRow($query)){
		$arr['id']        = $row['id'];
		$arr['title']     = $row['title'];
		$arr['ifred']     = $row['ifred'];
	...
	$arr = '';
	$czf = array();
	$query = $db -> query("SELECT a.cityid,a.id,a.title,a.ifred,a.ifbold,a.img_path,a.dir_typename FROM `{$db_qq3479015851}information` AS a WHERE ".get_children($tpl_index[portal][zufang])." AND (a.info_level = 1 OR a.info_level = 2) {$city_limit} ORDER BY a.begintime DESC LIMIT 0,4");
	while($row = $db -> fetchRow($query)){
		$arr['id']        = $row['id'];
		$arr['title']     = $row['title'];
		$arr['ifred']     = $row['ifred'];
	...
	$query	 = $db -> query("SELECT a.cityid,a.id,a.title,a.ifred,a.ifbold,a.begintime,a.catname,a.dir_typename FROM {$db_qq3479015851}information AS a  WHERE ".$inershou." AND (a.info_level = 1 OR a.info_level = 2) {$city_limit} ORDER BY a.begintime DESC LIMIT 0,10");
	while($row   = $db -> fetchRow($query)){
		$arr['id'] 	      = $row['id'];
		$arr['title'] 	  = $row['title'];
	...
	$arr = '';
	$query = $db -> query("SELECT a.cityid,a.id,a.title,a.img_path,a.dir_typename FROM `{$db_qq3479015851}information` AS a WHERE ".$inershou." AND (a.info_level = 1 OR a.info_level = 2) AND img_path != '' {$city_limit} ORDER BY a.begintime DESC LIMIT 0,6");
	if($query){
		while($row = $db -> fetchRow($query)){
			$arr['id']        = $row['id'];
			$arr['title']     = $row['title'];
	...
	if(is_array($index_cat)){
		foreach($index_cat as $firstcatkey => $firstcatval){
			$idin = get_page_idin2("id","SELECT a.id FROM `{$db_qq3479015851}information` AS a WHERE a.gid='$firstcatval[catid]' AND (a.info_level = 1 OR a.info_level = 2) {$city_limit} ORDER BY a.begintime DESC",$tpl_index['foreachinfo']);
			if($idin){
				$query	= $db -> query("SELECT a.cityid,a.catname,a.dir_typename,a.id,a.userid,a.catid,a.title,a.ifred,a.ifbold,a.begintime FROM `{$db_qq3479015851}information` AS a WHERE a.id IN ($idin) ORDER BY a.begintime DESC");
