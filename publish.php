...
if($act == 'dopost') {
	...
	if($action == 'input'){
		...
		if($yet = $db->getOne("SELECT begintime FROM `{$db_qq3479015851}information` WHERE title='$title' AND tel = '$tel' ORDER BY id DESC")){
		...
		if (empty($ismember)){
			...
			/*信息认证情况*/
			if($userid){
				$row = $db ->getRow("SELECT per_certify,com_certify FROM `{$db_qq3479015851}member` WHERE userid = '$userid'");
			...
			if($score_changer){
				$db->query("UPDATE `{$db_qq3479015851}member` SET score = score".$score_changer." WHERE userid = '$userid'");
			}
				...
				for ($i = 0; $i < $img_count; $i++) {
					if ($images[$i]) {
						...
						if($qiniuopen==0){
							...
							$qiniu_sep = $qiniusettings['separate'];
							$qiniu_ori = $qiniusettings['originalPic'];
	...
	if($catname && !$catid) $catid = $db ->getOne("SELECT catid FROM `{$db_qq3479015851}category` WHERE catname = '$catname'");
	/*如果为分类选择页*/
	if(!$catid){
		...
		include qq3479015851_tpl('info_post');
		...
		if($cat['parentid'] == 0) {
			...
			include qq3479015851_tpl('info_post');
			exit;
		}elseif($db->getOne("SELECT COUNT(catid) FROM `{$db_qq3479015851}category` WHERE parentid = '$catid'")){
			//如果为最底层分类
		...
		/*验证回答设置*/
		$whenpost = '';
		$whenpost = $db -> getOne("SELECT value FROM `{$db_qq3479015851}config` WHERE description = 'whenpost' AND type = 'checkanswe'");
		...
		globalassign();
		include qq3479015851_tpl('info_post_write');
	}
		...
		$cat = $db->getRow("SELECT a.if_upimg,a.modid,b.catid FROM `{$db_qq3479015851}category` AS a LEFT JOIN `{$db_qq3479015851}category` AS b ON a.parentid = b.catid WHERE a.catid = '$catid'");
		...
		if($post['ismember'] == 1){
			...
			if(qq3479015851_count("information","WHERE id = '$id' AND manage_pwd = '".md5($manage_pwd)."' AND ismember = 0") == 0){
		...
		如不修改，请留空</td></tr>";
			...
			$post['part'] = $part;
			globalassign();
			include qq3479015851_tpl('info_write_pwd');
			exit;
		}
		...
		$post['qq3479015851_extra_value']   = is_array($post['qq3479015851_extra_value']) ? $post['qq3479015851_extra_value'] : array();
		...
		$r	= $db ->getRow("SELECT a.cityid,b.dir_typename FROM `{$db_qq3479015851}information` AS a LEFT JOIN `{$db_qq3479015851}category` AS b ON a.catid = b.catid WHERE a.id = '$ok[id]'");
...
function post_cat_info($catid){
	...
	return $db -> getRow("SELECT a.catid,a.modid,a.if_upimg,a.catname,b.catid as parentid,a.if_mappoint,b.catname as parentname FROM `{$db_qq3479015851}category` AS a LEFT JOIN `{$db_qq3479015851}category` AS b ON a.parentid = b.catid WHERE a.catid = '$catid'");
}
?>