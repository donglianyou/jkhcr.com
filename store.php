...
if($action != 'dopost'){
	...
	$store['good_comment'] = $db -> getOne("SELECT COUNT(a.id) FROM `{$db_qq3479015851}member_comment` AS a WHERE a.userid = '$store[userid]'  AND enjoy IN('2','3') ");
	$store['soso_comment'] = $db -> getOne("SELECT COUNT(a.id) FROM `{$db_qq3479015851}member_comment` AS a WHERE a.userid = '$store[userid]' AND enjoy = '1' ");
	$store['bad_comment'] = $db -> getOne("SELECT COUNT(a.id) FROM `{$db_qq3479015851}member_comment` AS a WHERE a.userid = '$store[userid]' AND enjoy = '0' ");
	...
	if($part == 'about'){
		...
		$sql = "SELECT a.* FROM {$db_qq3479015851}goods AS a $where ORDER BY a.goodsid DESC";
		...
		if(!$id && $typeid){
			...
			$sql = "SELECT a.*,b.id AS uid FROM {$db_qq3479015851}member_docu AS a LEFT JOIN `{$db_qq3479015851}member` AS b ON a.userid = b.userid  $where ORDER BY a.id DESC";
			
			$rows_num = $db->getOne("SELECT COUNT(a.id) FROM `{$db_qq3479015851}member_docu` AS a {$where}");
			...
			if(!$docu = $db->getRow("SELECT a.* FROM `{$db_qq3479015851}member_docu` AS a WHERE a.userid = '$store[userid]' AND a.id = '$id'")){
				die('您所指定的空间文档不存在');
			} else {
				$db->query("UPDATE `{$db_qq3479015851}member_docu` SET hit = hit + 1 WHERE id = '$id'");
				$docutype = get_member_docutype();
		...
		$rows_num = $db -> getOne("SELECT COUNT(a.id) FROM `{$db_qq3479015851}member_album` AS a $where");
		$album 	  = page1("SELECT a.* FROM `{$db_qq3479015851}member_album` AS a $where ORDER BY a.id desc",15);
		...
		$comment = array();
		$result = page1("SELECT a.* FROM `{$db_qq3479015851}member_comment` AS a $where AND a.commentlevel = '1' order by id DESC",15);
		...
		$album = $db -> getAll("SELECT a.* FROM `{$db_qq3479015851}member_album` AS a  WHERE a.userid='$store[userid]' ORDER BY a.id DESC LIMIT 0,15");
		$where = " WHERE a.userid = '$store[userid]'";
		$store['location'] = get_store_location($uri['index'],$store['tname'],'机构首页');
		$goods = qq3479015851_get_goods(8,1,'','',$store['userid']);
	...
	if($part == 'comment'){
		...
		$face 			= $db -> getOne("SELECT prelogo FROM `{$db_qq3479015851}member` WHERE userid = '$fromuser'");
		$face			= $face ? $face : '';
		$db -> query("INSERT INTO `{$db_qq3479015851}member_comment` (id,userid,fromuser,content,commentlevel,quality,service,environment,price,avgprice,enjoy,pubtime,face) VALUES ('','$userid','$fromuser','$content','$commentlevel','$quality','$service','$environment','$price','$avgprice','$enjoy','$timestamp','$face')");
		$uid = $db -> getOne("SELECT id FROM `{$db_qq3479015851}member` WHERE userid = '$user'");
		if($commentlevel == '0'){
